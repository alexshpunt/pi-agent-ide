...
{
    const runGate: IdePostEditGateRunner = vi.fn((filePath, toolchain) =>
    {
...
test("ignores resources without an absolute filesystem identity", async () =>
{
    ...
    await expect(runIdePostEditGate(transaction("memory:example.ts"), runGate)).resolves.toBeUndefined();
