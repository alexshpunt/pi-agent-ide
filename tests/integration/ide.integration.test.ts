...
test("loads IDE extensions together while preserving read integrations", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        expect(prompt).toContain("Always use `ast:<path>` for the first read");
        expect(prompt).toContain("resolves `symbol:<path>#<selector>`, `graph:<path>`, and `graph:<path>#<selector>`");
        expect(prompt).toContain("- `lint` — Adds lint diagnostics to textual read results without applying fixes.");
    });
}, 60_000);
