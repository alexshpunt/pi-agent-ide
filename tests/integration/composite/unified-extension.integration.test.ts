...
const ideExtensionPath = process.env.PI_AGENT_IDE_TEST_EXTENSION ?? path.resolve("src/pi-agent-ide.ts");
const externalPluginPath = process.env.PI_AGENT_IDE_TEST_EXTERNAL_PLUGIN
...
test("loads one IDE extension, applies its config, and accepts an external plugin", async () =>
{
    ...
    expect(getToolResultMessage(result, disabledCall).details).toMatchObject({ failure: { code: "NO_RESOLVER" } });
