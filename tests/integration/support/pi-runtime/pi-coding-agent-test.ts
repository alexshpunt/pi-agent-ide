...
function includeReadTool(tools: ToolSelection | undefined): ToolSelection | undefined
{
    ...
    const include = selection.include === undefined ? undefined : [...new Set([...selection.include, "read"])];
...
function editedFiles(conversation: readonly ReturnType<typeof assistantMessage>[]): readonly string[]
{
...
): readonly ReturnType<typeof assistantMessage>[]
{
    ...
    const preflight = files.map((filePath, index) => readMessage(`${idPrefix}-read-preflight-${index}`, filePath));
    const mutations = conversation.slice(0, -1).map((message, index) =>
        index === 0 ? withBoundary(message, "PREFLIGHT FINISHED") : message
    );
...
export class PiIntegrationTest extends BasePiIntegrationTest
{
    public constructor(options: PiIntegrationTestOptions)
    {
        ...
        super({
            ...
            conversation: withRequiredReadPreflightPostflight(options.conversation ?? [], options.testName),
