        ...
        await withTempWorkspace(async (directory) =>
        {
            ...
            const result = await new PiIntegrationTest({
                ...
                conversation: [
                    ...
                    assistantMessage([text("The large write and guarded overwrite demo is complete", { delayMs: 0 })]),
                ],
            ...
            expect(getToolResultText(result, blockedOverwriteCallId)).toContain("Reason: The file already exists");
            ...
            const overwriteMutation = getTextToolMutationData(getToolExecutionDetails(confirmedOverwriteExecution));
            expect(overwriteMutation).toMatchObject({
                beforeContentMap: { [demoFileName]: initialContent },
                afterContent: overwriteContent,
            });
            await expect(readFile(path.join(directory, demoFileName), "utf8")).resolves.toBe(overwriteContent);
