...
describe("interactive text editor demos", () =>
{
    test("streams a large range replacement with a moving cursor", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const commentLine = requiredLine(lines, "     * Undo the newest N entries globally or in one file.");
            ...
            const result = await new PiIntegrationTest({
                ...
                conversation: [
                    assistantMessage([
                        toolCall({ id: callId, name: "replace", arguments: arguments_, ...interactivePacing }),
                    ], { stopReason: "toolUse" }),
