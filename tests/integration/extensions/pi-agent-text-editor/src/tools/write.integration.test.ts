...
describe("pi-agent-text-editor tools", () =>
{
    test("registers the text tools and writes a file", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(result.traceEvents.find((event) => event.type === "tools_configured")?.activeTools).toEqual(tools);
            expect(getToolExecution(result, mutationCallId).isError).toBe(false);
            expect(getProviderSystemPrompt(result)).toContain("# Available Text Anchors");
