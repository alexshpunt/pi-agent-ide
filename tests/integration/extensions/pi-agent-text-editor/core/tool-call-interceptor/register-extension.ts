...
export default function registerToolInterceptorIntegrationExtension(pi: ExtensionAPI): void {
  ...
  registerToolCallInterceptor(pi, {
    ...
    intercept: (context) =>
      context.partialArgs ? block(`partial:${String(context.partialArgs.value)}`) : undefined,
  });
  ...
  registerToolCallInterceptor(pi, {
    ...
    intercept: (context) => {
      ...
      return context.args.value === "verify"
        ? block(isAbortObserved ? "abort-cleanup-observed" : "abort-cleanup-missing")
        : undefined;
    },
    onAbort: () => {
  ...
  registerToolCallRecovery(pi, {
    ...
    execute: async (parameters, context) => {
      await writeFile(path.join(context.cwd, "recovered.json"), JSON.stringify(parameters));
      return { content: [{ type: "text", text: "recovered" }], details: parameters };
    },
  });
  registerToolCallInterceptor(pi, {
    name: "recovery-block",
