...
describe("pi-agent-text-editor batch recovery", () =>
{
                ...
                conversation: [
                    assistantMessage([
                        ...
                        call("copy", "mixed-copy", {
                            ...
                            targetStart: formatLineHashAnchor(9, "    return { mode, retries, timeout, endpoint, headers };"),
                        }),
                    ], { stopReason: "toolUse" }),
                    assistantMessage([text("done")]),
                ],
    ...
    test("applies every recoverable call against the original snapshots", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolExecutionDetails(getToolExecution(result, "atomic-stale"))).toEqual(expect.objectContaining({
                batchRecovery: expect.objectContaining({ state: "failed-not-applied", recovered: false }),
            }));
    ...
    test("keeps completed streamed calls and aborts the unfinished stale body", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const source = ["alpha", "first", "second", "third", "fourth", "fifth", "sixth", "omega"].join("\n");
            ...
            const tailChunks = tailCalls.map(({ id, line, original }, index) => ({
                ...
                argumentsJson: `{"start":${JSON.stringify(formatLineHashAnchor(line, original))},"text":${
                    JSON.stringify(
                        `${id}\n${staleText}`,
                    )
                }}`,
            ...
            const deltas = result.traceEvents.flatMap((trace) =>
            {
                ...
                return update?.type === "toolcall_delta" && typeof update.delta === "string" ? [update.delta] : [];
            });
