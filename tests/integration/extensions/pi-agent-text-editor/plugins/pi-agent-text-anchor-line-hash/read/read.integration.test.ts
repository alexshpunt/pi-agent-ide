...
import { formatLineHashAnchor, renderLineHashLines } from "pi-agent-text-anchor-line-hash/api/anchor";
import {
    assistantMessage,
...
import type { ReadResultDetails, ReadTextLine } from "pi-agent-read/api/tools/read";
import { generateReadExtensions } from "pi-agent-read/testing";

interface TextEditorReadLine extends ReadTextLine
{
...
const changedFixtureLines = fixtureLines.with(
    7,
    "The changed observation produces a different content-derived anchor.",
);
...
test("adds hash anchors to filesystem text", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        const firstLine = original.details?.lines?.[0] as TextEditorReadLine | undefined;
        expect(firstLine).toMatchObject({ lineNumber: 1, content: fixtureLines[0] });

        const prompt = getProviderSystemPrompt(result);
        expect(prompt).toContain("- `text-anchor-line-hash` — Adds `LINE#HASH` anchors to textual read results.");
    });
});
...
test("keeps original anchors when projecting a range", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        expect(getToolResultText(result, "read-range"))
            .toContain(renderLineHashLines(fixtureLines, 7, 9).join("\n"));
...
test("does not process text resolved by another plugin", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        const lines = read.details?.lines as readonly TextEditorReadLine[] | undefined;
