...
describe("text mutation renderer", () =>
{
    ...
    test("streams a diff for a mutation with an inherited path", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            const source = Array.from({ length: 24 }, (_, index) => `const value${index + 1} = ${index + 1};`).join(
                "\n",
            );
    ...
    test("keeps one final viewport for every batched mutation call", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            const source = Array.from({ length: 32 }, (_, index) => `const value${index + 1} = ${index + 1};`).join(
                "\n",
            );
    ...
    test("keeps the generated viewport after post-edit formatting", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            const before = Array.from({ length: 30 }, (_, index) => `const value${index + 1} = ${index + 1};`).join(
                "\n",
            );
            ...
            await expect(readFile(file, "utf8")).resolves.toContain("// formatted outside generated viewport");
    ...
    test("restores the semantic viewport from the persisted session", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            {
                throw new Error("Expected the mutation run to persist a session");
            }

            const sessionFile = path.join(firstRun.artifacts.sessionDirectory, sessionName);
            const sessionEntries = (await readFile(sessionFile, "utf8"))
                .trim()
                .split("\n")
                .map((line) => JSON.parse(line) as PersistedSessionEntry);
            const persistedResult = sessionEntries.find((entry) =>
                entry.type === "message" && entry.message?.role === "toolResult" && entry.message.toolName === "replace"
            );
    ...
    test("renders a realistic mixed TypeScript hunk with semantic stats", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const result = await runSingleMutation(directory, "write", { path: "service.ts", content: after });
            const rendered = result.tuiRenderedOutput;
    ...
    test.each([
        ...
        {
            tool: "delete",
            arguments: { path: "subject.ts", start: formatLineHashAnchor(2, "    const value = load();") },
            stats: "+0 ~0 -1",
            row: /2\s+-\s+const value = load\(\);/u,
        },
    ])("renders a pure semantic row through $tool", async ({ tool, arguments: input, stats, row }) =>
    {
        await withTempWorkspace(async (directory) =>
        {
            await createFixture(
    ...
    test("shows per-file stats for a copy with a target", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(rendered).toMatch(/copy source\.txt:\S+\s+\+0 ~0 -0\s+->\s+target\.txt:\S+\s+\+1 ~0 -0/u);
    ...
    test("shows per-file stats for a move with a target", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(rendered).toMatch(/move source\.txt:\S+\s+\+0 ~0 -1\s+->\s+target\.txt:\S+\s+\+1 ~0 -0/u);
            expect(panels).toHaveLength(2);
    ...
    test.each(["before content starts", "when arguments arrive together"])(
        ...
        async (delivery) =>
        {
            await withTempWorkspace(async (directory) =>
            {
                ...
                const generated = Array.from({ length: 12 }, (_, index) => `blocked line ${index + 1}`).join("\n");
                const argumentsJson = JSON.stringify({ path: "blocked-overwrite.txt", content: generated });
                const chunks = delivery === "before content starts"
                ...
                expect(getToolResultText(result, "blocked-overwrite")).toContain("Reason: The file already exists");
...
async function runCrossFileMutation(directory: string, tool: "copy" | "move", selectedText: string)
{
    return new PiIntegrationTest({
