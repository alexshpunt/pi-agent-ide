...
const staleDeliveries: readonly { readonly name: string; readonly chunks: ChunkSpec; }[] = [
    {
        ...
        chunks: {
            kind: "explicit",
            chunks: [
                "{\"path\":\"subject.txt\",\"anchor\":\"1#AAAA\",",
                "\"text\":\"inserted\"}",
            ],
        },
    },
...
function toolCallDeltaCount(result: Awaited<ReturnType<typeof runBlockedInsert>>, chunks: ChunkSpec): number
{
...
describe("pi-agent-text-editor stale anchor", () =>
{
    ...
    test.each(staleDeliveries)("blocks a stale anchor $name", async ({ name, chunks }) =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolCallNames(result).filter((tool) => tool === "insert")).toEqual(["insert", "insert"]);
    ...
    test("blocks two consecutive streamed stale anchors", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolCallNames(result).filter((tool) => tool === "insert"))
                .toEqual(["insert", "insert", "insert"]);
            await expect(readFile(file, "utf8")).resolves.toBe("alpha\nrecovered\nbeta");
        });
    });
});
