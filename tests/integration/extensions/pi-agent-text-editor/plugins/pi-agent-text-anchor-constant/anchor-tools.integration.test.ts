...
): Promise<void>
{
    await withTempWorkspace(async (directory) =>
    {
        ...
        const toolArguments = tool === "copy" || tool === "move"
        ...
        expect(systemPrompt).toContain("### file-position");
        expect(systemPrompt).toContain("`begin` resolves to the first existing line.");
        expect(systemPrompt).toContain("`end` resolves to the last existing line.");
