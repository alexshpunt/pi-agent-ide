...
const completeOverwriteArgumentsJson = JSON.stringify({ path: "write.txt", content: largeOverwriteContent });
const overwriteDeliveries: readonly {
...
}[] = [
    {
        ...
        chunks: {
            ...
            chunks: [
                "{\"path\":\"write.txt\",",
                `\"content\":${JSON.stringify(largeOverwriteContent)}}`,
            ],
        },
...
function toolCallDeltaCount(result: Awaited<ReturnType<typeof runBlockedWrite>>, chunks: ChunkSpec): number
{
    ...
    {
        if (trace.type !== "message_update" || typeof trace.event !== "object" || trace.event === null)
        {
            return false;
        }
        ...
        return update?.type === "toolcall_delta" && typeof update.delta === "string" && expected.has(update.delta);
    }).length;
}

describe("pi-agent-text-editor overwrite guard", () =>
{
    test.each(overwriteDeliveries)("blocks a write $name", async ({ name, chunks, expectedDeltaCount }) =>
    {
        await withTempWorkspace(async (directory) =>
        {
            const file = await createFixture(directory, "write.txt", "before\n");
            const callId = name.includes("streaming") ? "overwrite-write-streamed" : "overwrite-write-complete";
            ...
            expect(getToolCallNames(result).filter((tool) => tool === "write")).toEqual(["write", "write"]);
    ...
    test("blocks replace when resolved anchors cover the complete file", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolResultText(scenario.result, scenario.mutationCallId)).not.toContain("MUTATION_REJECTED");
    ...
    test("blocks streamed replace before replacement text is generated and keeps recovery", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const argumentsJson = JSON.stringify({ path: "replace-streamed.txt", start, end, text: replacement });
            const boundaryArguments = JSON.stringify({ path: "replace-streamed.txt", start, end });
            const chunks: ChunkSpec = {
                kind: "explicit",
                chunks: [
                    `${boundaryArguments.slice(0, -1)},`,
            ...
            expect(getToolExecution(result, callId).isError).toBe(true);
            expect(getToolResultText(result, callId)).toContain("Reason: The file already exists");
            expect(getToolCallNames(result).filter((tool) => tool === "replace")).toEqual(["replace", "replace"]);
            expect(getToolExecution(result, `${callId}-recovery`).isError).toBe(false);
    ...
    test.each(["copy", "move"] as const)("blocks %s when its target range covers the complete file", async (tool) =>
    {
        await withTempWorkspace(async (directory) =>
        {
