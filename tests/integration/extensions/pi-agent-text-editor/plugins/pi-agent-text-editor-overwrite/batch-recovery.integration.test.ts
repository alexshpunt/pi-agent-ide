...
function batchId(details: unknown): string | undefined
{
    ...
    return typeof batch === "object" && batch !== null && "batchId" in batch && typeof batch.batchId === "string"
        ? batch.batchId
        : undefined;
}
...
)
{
    ...
    const result = await new PiIntegrationTest({
        ...
        conversation: [
            assistantMessage([
                call(tool, blockedId, arguments_),
                call(tool, skippedId, arguments_),
            ], { stopReason: "toolUse" }),
...
describe("pi-agent-text-editor overwrite batch recovery", () =>
{
    test("stops before a repeated write overwrite", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            await runImmediateAbort(directory, "overwrite-write-abort", "write", {
                path: "write.txt",
                content: "after\n",
            }, 1);
    ...
    test("stops before a repeated full-file replace", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            await runImmediateAbort(directory, "overwrite-replace-abort", "replace", {
                path: "replace.txt",
    ...
    test("stops before a repeated copy into a full target range", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const target = await createFixture(directory, "copy-target.txt", "target-first\ntarget-last\n");

            await runImmediateAbort(directory, "overwrite-copy-abort", "copy", {
    ...
    test("stops before a repeated move and leaves both files untouched", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const target = await createFixture(directory, "move-target.txt", "target-first\ntarget-last\n");

            await runImmediateAbort(directory, "overwrite-move-abort", "move", {
    ...
    test("keeps successful same-file replacements when a later overwrite is blocked", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolResultText(result, "same-overwrite")).toContain("Reason: The file already exists");
    ...
    test("keeps successful replacements on other files when an overwrite is blocked", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolResultText(result, "different-overwrite")).toContain("Reason: The file already exists");
    ...
    test("keeps same-file replacements before an overwrite and skips later calls", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            expect(getToolResultText(result, "recovery-overwrite")).toContain("Reason: The file already exists");
