...
test("reads relative, absolute, empty, and ranged UTF-8 text when loaded before the provider", async () => {
  await withTempDirectory(async (directory) => {
    ...
    expect(
      getToolResultText(await runRead(directory, "notes.txt", "filesystem-text-relative")),
    ).toBe(textFixture);
    expect(
      getToolResultText(
        await runRead(path.join(directory, "workspace"), outside, "filesystem-text-absolute"),
      ),
    ).toBe("absolute text\n");
    ...
    expect(
      getToolResultText(
        await runRead(directory, "notes.txt", "filesystem-text-range", { offset: 2, limit: 1 }),
      ),
    ...
    expect(
      getToolResultText(
        await runRead(directory, "tail.txt", "filesystem-text-tail-limit", {
          offset: -4,
          limit: 2,
        }),
      ),
...
test("reads file URLs and lists directories as read-only resources", async () => {
  await withTempDirectory(async (directory) => {
    ...
    expect(
      getToolResultText(await runRead(directory, directory, "filesystem-text-directory")),
    ).toBe([`📁 ${directory}/`, "", "  📁 nested", "  📄 notes.txt"].join("\n"));

    expect(
      getToolResultText(
        await runRead(
          directory,
          pathToFileURL(path.join(directory, "notes.txt")).href,
          "filesystem-text-file-url",
        ),
      ),
    ).toBe("hello\n");
  });
...
test("reports malformed bytes through the read contract", async () => {
  await withTempDirectory(async (directory) => {
    ...
    const result = await runRead(directory, "invalid.bin", "filesystem-text-invalid-utf8");
