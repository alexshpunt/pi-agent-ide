...
const demoExtensions = [
    ...
    path.resolve("tests/integration/extensions/pi-agent-search/interactive-demos/fixtures/web-search-transport.ts"),
...
const calls = [
    ...
    { id: "search-semantic", query: "semantic:protocol atlas", resolverId: "semantic", arguments: { limit: 3 } },
    { id: "search-web", query: "web:Pi Search resolver architecture", resolverId: "web", arguments: { limit: 3 } },
    { id: "search-lsp", query: "lsp:createResolverAtlas", resolverId: "lsp", arguments: { limit: 5 } },
    { id: "search-ast", query: "ast:createResolverAtlas()", resolverId: "ast", arguments: { path: "src" } },
] as const;
    ...
    test("shows every installed resolver through one Search tool", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            for (const call of calls)
            {
                ...
                expect((getToolExecutionDetails(execution) as { resolverId?: string; }).resolverId, call.id)
            ...
            expect(getToolResultText(result, "search-web")).toContain("https://example.com/pi-search-resolvers");
...
async function prepareWorkspace(directory: string): Promise<void>
{
            ...
            JSON.stringify({
                servers: {
                    "typescript-language-server": {
                        command: [path.resolve("node_modules/typescript-7/bin/tsc"), "--lsp", "--stdio"],
    ...
    await promisify(execFile)(
        path.resolve("src/extensions/pi-agent-search/plugins/pi-agent-search-semantic/node_modules/.bin/qmd"),
        ["update"],
        { cwd: directory },
    );
}
