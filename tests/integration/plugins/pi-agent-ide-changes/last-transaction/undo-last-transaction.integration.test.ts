...
const overwriteExtension =
    path.resolve("src/extensions/pi-agent-text-editor/plugins/pi-agent-text-editor-overwrite/index.ts");
const undoExtension = path.resolve("src/plugins/pi-agent-ide-changes/index.ts");
const outsideWriteExtension =
    path.resolve("tests/integration/plugins/pi-agent-ide-changes/last-transaction/support/register-outside-write.ts");
const runtimeExtensions = [...extensions.paths, overwriteExtension, undoExtension, outsideWriteExtension];
...
describe("undo last text transaction", () =>
{
    test("restores one direct edit and does not expose redo", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const result = await runScenario(directory, "undo-last-direct", [
            ...
            expect(getToolResultText(result, "direct-redo-attempt")).toContain("No last text transaction");
    ...
    test("uses one snapshot for a batch and keeps files independent", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const result = await runScenario(directory, "undo-last-batches", [
                toolMessage(
                    toolCall({
                        id: "same-batch-first",
                        name: "replace",
    ...
    test("keeps only the newest edit and rejects stale or new files", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            const result = await runScenario(directory, "undo-last-guards", [
                toolMessage(toolCall({
                    id: "latest-first",
                    name: "insert",
            ...
            expect(getToolResultText(result, "latest-older-attempt")).toContain("No last text transaction");
            ...
            expect(getToolResultText(result, "created-undo-attempt")).toContain("No last text transaction");
    ...
    test("selective undo clears the saved transaction", async () =>
    {
        await withTempWorkspace(async (directory) =>
        {
            ...
            {
                throw new Error("The selective undo fixture did not produce an undo group");
            }

            const result = await runScenario(directory, "undo-last-after-selective", [
                toolMessage(toolCall({
            ...
            expect(getToolResultText(result, "selective-last-attempt")).toContain("No last text transaction");
