...
const supportRoot = path.resolve(
    "tests/integration/plugins/pi-agent-ide-changes/read/support",
);
...
test("read refreshes anchors from the current file and skips clean and untracked files", async () =>
{
    await withRepository(async ({ directory, fileName }) =>
    {
        ...
        const refreshed = await runReads(directory, "undo-read-refreshed", [readCall("refreshed", fileName)]);
        const refreshedAnchor = getToolResultText(refreshed, "refreshed").match(/CHANGE#[A-Z0-9]+/u)?.[0];
...
test("undo restores one read change and leaves unrelated changes visible", async () =>
{
    await withRepository(async ({ directory, fileName }) =>
    {
        const baseline = await committedSource();
        const current = await changedSource();
        const selector = selectorForCurrentMarker(fileName, baseline, current, "const DEFAULT_ATTEMPTS = 3;");
        await writeFile(path.join(directory, fileName), current, "utf8");
        ...
        expect(getToolResultText(result, "undo-selected")).toContain("-|        |const DEFAULT_ATTEMPTS = 3;");
        expect(getToolResultText(result, "undo-selected")).toMatch(
            /\+\|\s*25#[A-Z0-9]{4}\|const DEFAULT_ATTEMPTS = 2;/u,
        );
        ...
        expect(panel).toMatch(/25\s+~\s+const DEFAULT_ATTEMPTS = 2;/u);
...
test("batched undo calls inherit the file and restore independent read changes", async () =>
{
    await withRepository(async ({ directory, fileName }) =>
    {
        const baseline = await committedSource();
        const current = await changedSource();
        const attemptsSelector = selectorForCurrentMarker(fileName, baseline, current, "const DEFAULT_ATTEMPTS = 3;");
...
function readCall(id: string, fileName: string, range: { offset: number; limit: number; } | undefined = undefined)
{
    return toolCall({
        id,
        name: "read",
...
async function runReads(directory: string, testName: string, calls: readonly ReturnType<typeof readCall>[])
{
...
function expectInlineHunk(rendered: string, marker: string, expectedFragments: readonly string[]): void
{
    ...
    const endIndex = lines.findIndex((line, index) =>
        index >= markerIndex && line.includes(`change-end: ${selector ?? "missing"}`)
    );
...
function selectorForCurrentMarker(repositoryPath: string, baseline: string, current: string, marker: string): string
{
...
{
    const lines = rendered.split("\n");
    const lineIndex = lines.findIndex((line) => line.slice(2).trimStart().startsWith(`${lineNumber}#`));
