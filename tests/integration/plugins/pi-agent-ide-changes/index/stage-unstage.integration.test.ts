...
{
    throw new Error("The stage fixture did not produce a change group");
}
...
test("stages and unstages one change, then removes a staged change with undo", async () =>
{
    await withTempWorkspace(async (directory) =>
    {
        ...
        for (const callId of ["stage-change", "read-staged", "unstage-change", "read-unstaged", "stage-again", "undo-staged"])
        {
...
{
    return assistantMessage([toolCall({ id, name, arguments: arguments_ })], { stopReason: "toolUse" });
}
