...
test("read shows LSP diagnostics without changing the source", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        expect(prompt).toContain("resolves `symbol:<path>#<selector>`, `graph:<path>`, and `graph:<path>#<selector>`");
    });
}, 60_000);
...
test("symbol and graph sources expose layered LSP code views", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        const prompt = getProviderSystemPrompt(result);
        expect(prompt).toContain("`symbol:<path>#<selector>`");
        expect(prompt).toContain("`graph:<path>`");
        expect(prompt).toContain("`graph:<path>#<selector>`");
    });
}, 60_000);
...
): Promise<void>
{
    ...
    await Promise.all(
        Object.entries(files).map(([fileName, source]) => writeFile(path.join(directory, fileName), source, "utf8")),
    );
}
