...
test("an edit returns diagnostics from the written TypeScript file", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        await writeFile(path.join(directory, "lsp-servers.json"), JSON.stringify({
            servers: {
                "typescript-language-server": {
                    command: [path.join(repoRoot, "node_modules/typescript-7/bin/tsc"), "--lsp", "--stdio"],
        ...
        expect(JSON.stringify(getToolExecution(scenario.result, scenario.mutationCallId).result)).toContain("compiler:2322");
