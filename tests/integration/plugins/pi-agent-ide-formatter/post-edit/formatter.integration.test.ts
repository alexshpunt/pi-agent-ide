...
import { expectTextToolDiff, runTextToolScenario } from "#integration-tests/support/pi-runtime/scenario.js";
...
test("an edit returns the formatter result instead of the requested intermediate text", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        await writeFile(
            path.join(directory, "formatters.json"),
            JSON.stringify({
                formatters: {
                    fixture: { extensions: [".ts"], command: ["node", formatterScript] },
                },
            }),
            "utf8",
        );
