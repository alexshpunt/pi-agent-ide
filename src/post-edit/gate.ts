...
import { computeChangedRanges, type DiagnosticHint } from "pi-agent-text-editor/api/mutation-result";

import type { ActiveToolchain, Diagnostic, GateResult } from "#src/toolchain/types.js";
import type { TextMutationResultContributionData, TextPostEditTransaction } from "pi-agent-text-editor/api/post-edit";
...
): Promise<TextMutationResultContributionData | undefined>
{
    ...
    const fileResult = gateResult.files.find((result) => result.filePath === transaction.resourceSource);
    ...
    {
        if (!Number.isInteger(diagnostic.line) || diagnostic.line < 1 || diagnostic.line > finalLines.length)
        {
            return;
        }
...
{
    return DIAGNOSTIC_SEVERITY_RANK[left.severity] - DIAGNOSTIC_SEVERITY_RANK[right.severity]
