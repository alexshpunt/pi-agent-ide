...
describe("built-in extension selection", () =>
{
    ...
    {
        expect(selectBuiltinExtensions(extensions, []).enabled.map(({ id }) => id))
            .toEqual(["core", "plugin", "feature", "independent"]);
    });
    ...
    {
        expect(() => selectBuiltinExtensions(extensions, ["missing"]))
            .toThrow("Unknown Pi Agent IDE extension ID in disabledExtensions: missing");
    });
});
