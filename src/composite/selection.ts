...
): { readonly enabled: readonly BuiltinExtension[]; readonly disabled: ReadonlySet<string>; }
{
    ...
    for (const extension of extensions)
    {
        for (const dependency of extension.dependencies)
        {
            ...
            {
                throw new Error(`Pi Agent IDE extension ${extension.id} has unknown dependency ${dependency}`);
            }
        }
    }
    ...
    while (changed)
    {
        ...
        for (const extension of extensions)
        {
            if (!disabled.has(extension.id) && extension.dependencies.some((id) => disabled.has(id)))
            {
