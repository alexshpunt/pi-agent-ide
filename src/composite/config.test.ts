...
describe("Pi Agent IDE config", () => {
  test("uses the Pi agent directory unless an explicit config path is set", () => {
    expect(resolvePiAgentIdeConfigPath({}, "/home/example")).toBe(
      "/home/example/.pi/agent/pi-agent-ide.json",
    );
    expect(resolvePiAgentIdeConfigPath({ PI_CODING_AGENT_DIR: "/agent" }, "/home/example")).toBe(
      "/agent/pi-agent-ide.json",
    );
    expect(
      resolvePiAgentIdeConfigPath({ PI_AGENT_IDE_CONFIG: "./custom.json" }, "/home/example"),
    ).toBe(path.resolve("custom.json"));
  });

  test("enables every built-in when the config file is absent", async () => {
    const directory = await temporaryDirectory();
    await expect(readPiAgentIdeConfig(path.join(directory, "missing.json"))).resolves.toEqual({
      disabledExtensions: [],
    });
  });
  ...
  test("reads disabled extension IDs", async () => {
    ...
    await writeFile(
      configPath,
      JSON.stringify({ disabledExtensions: ["search.semantic", "ide.lsp"] }),
    );
  ...
  test("rejects malformed disabled extension lists", async () => {
    ...
    await expect(readPiAgentIdeConfig(configPath)).rejects.toThrow(
