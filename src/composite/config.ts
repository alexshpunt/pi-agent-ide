...
export function resolvePiAgentIdeConfigPath(
  ...
  const configured = environment.PI_AGENT_IDE_CONFIG?.trim();
...
export async function readPiAgentIdeConfig(configPath: string): Promise<PiAgentIdeConfig> {
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON in Pi Agent IDE config at ${configPath}: ${errorMessage(error)}`,
      {
        cause: error,
      },
    );
  ...
  if (
    ...
    disabled.some((id) => typeof id !== "string" || id.length === 0)
  ) {
