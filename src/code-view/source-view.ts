...
export function formatSourceViewResults(
  ...
  if (
    results.length === 1 &&
    requiredValue(results[0]).status === "failed" &&
    requiredValue(results[0]).path === ""
  ) {
    const result = requiredValue(results[0]);
    const recovery = options.recovery === undefined ? [] : ["", "Recovery:", ...options.recovery];
