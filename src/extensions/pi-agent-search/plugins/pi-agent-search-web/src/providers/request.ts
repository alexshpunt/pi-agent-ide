import { providerUrl } from "./endpoints.ts";
...
function resolveDomainFilters(
  ...
  if (configAllowed) {
    const narrowed = requestAllowed
      ...
      : configAllowed;
    const allowed = requestBlocked
      ? narrowed.filter((domain) => !requestBlocked.includes(domain))
      : narrowed;
  ...
  if (configBlocked) {
    ...
    if (requestAllowed) {
      return {
        allowedDomains: nonEmptyDomains(
          requestAllowed.filter((domain) => !blocked.includes(domain)),
        ),
      };
    }
  ...
  if (requestAllowed) {
    return { allowedDomains: unique(requestAllowed) };
  }

  if (requestBlocked) {
    return { blockedDomains: unique(requestBlocked) };
  }
...
export function buildSearchRequest(
  ...
  if (config.provider === "tavily") {
    ...
    return {
      ...
      init: {
        method: "POST",
        headers: contentHeaders({ Authorization: `Bearer ${config.apiKey ?? ""}` }),
      },
      body,
    };
  }
  ...
  if (config.provider === "parallel") {
    ...
    if (allowedDomains) {
      sourcePolicy.include_domains = allowedDomains;
    }

    if (blockedDomains) {
      sourcePolicy.exclude_domains = blockedDomains;
    }
    ...
    if (config.model) {
      body.client_model = config.model;
    }
  ...
  if (config.provider === "z-ai") {
    if (config.model) {
      ...
      if (allowedDomains?.[0]) {
        webSearch.search_domain_filter = allowedDomains[0];
      }

      if (config.searchContextSize) {
        webSearch.content_size = config.searchContextSize;
      }
    ...
    if (allowedDomains?.[0]) {
      body.search_domain_filter = allowedDomains[0];
    }
  ...
  if (config.provider === "perplexity") {
    if (config.model) {
      ...
      if (config.searchContextSize) {
        body.web_search_options = { search_context_size: config.searchContextSize };
      }
  ...
  if (config.searchContextSize) {
    webSearchTool.search_context_size = config.searchContextSize;
  }
  ...
  if (config.userLocation) {
    webSearchTool.user_location = { type: "approximate", ...config.userLocation };
  }
