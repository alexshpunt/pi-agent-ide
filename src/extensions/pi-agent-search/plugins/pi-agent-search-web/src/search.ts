...
function extractErrorDetail(payload: unknown, bodyText: string): string
{
    if (isJsonObject(payload))
    {
        ...
        {
            return truncate(error, MAX_ERROR_DETAIL_LENGTH);
        }
        ...
        if (isJsonObject(error))
        {
            ...
            {
                return truncate(message, MAX_ERROR_DETAIL_LENGTH);
            }
        }
...
{
    if ("entryId" in entry && entry.entryId)
    {
        return `${entry.provider}/${entry.entryId}`;
    }
...
function selectOrder(strategy: RoutingStrategy, providers: SearchProviderEntry[], state: SearchRoutingState): number[]
{
    ...
    {
        return sortedPriorityIndices(providers);
    }
...
): Promise<SearchDetails>
{
    ...
    const results = normalizeSearchResponse(config.provider, payload).filter((item) => isWebUrl(item.url));
...
function attemptFromDetails(details: SearchDetails): SearchAttempt
{
    ...
    if (details.entryId)
    {
        attempt.entryId = details.entryId;
    }
...
): Promise<SearchDetails>
{
    ...
    for (const index of order)
    {
        ...
        if (details.error || details.results.length === 0)
        {
            if (!config.fallback)
            {
        ...
        {
            if (collected.size >= request.maxResults)
            {
                break;
            }
    ...
    if (collected.size > 0 && selectedDetails)
    {
        ...
        if (selectedDetails.entryId !== undefined)
        {
            details.entryId = selectedDetails.entryId;
        }
    ...
    return {
        ...
        error: `All configured search providers failed: ${
            attempts
                .map((attempt) => `${entryLabel(attempt)} ${attempt.error ?? "returned no usable results"}`)
                .join("; ")
        }`,
    };
}
