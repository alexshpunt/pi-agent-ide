...
import { createSearchRoutingState, formatSearchText, performSearch, type SearchRoutingState } from "#src/search.ts";

import type { SearchDetails } from "#src/types.ts";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import type { SearchResolver } from "pi-agent-search/api/search";

export default function registerWebSearch(pi: ExtensionAPI): void | Promise<void>
{
    ...
    const resolver: SearchResolver = {
        ...
        async tryResolve(request, context)
        {
            ...
            const nextKey = `${config.strategy}:${
                config.providers.map((provider) => provider.id ?? provider.provider).join("|")
            }`;

            if (
                routingState === undefined || routingKey !== nextKey
            ...
            const details = await performSearch(config, { query, maxResults }, context.signal, routingState);
