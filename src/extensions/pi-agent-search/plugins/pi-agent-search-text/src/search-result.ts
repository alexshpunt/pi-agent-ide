...
): SearchToolDetails
{
    ...
    const files = [...matchesBySource.entries()]
...
export function isSearchToolDetails(value: unknown): value is SearchToolDetails
{
    ...
    return value.fileCount === files.length
...
): SearchResultFile
{
    ...
    for (const match of matches)
    {
        ...
        {
            throw new Error(`Search results disagree about line ${String(match.lineNumber)} in ${source}.`);
        }
    ...
    const resultLines = [...lines.values()]
        ...
        {
            const ranges = [...line.ranges].sort((left, right) => left.from - right.from || left.to - right.to);
            return {
...
function isSearchResultFile(value: unknown): value is SearchResultFile
{
    ...
    return value.matchCount === (value.lines as readonly SearchResultLine[])
        .reduce((count, line) => count + line.matchCount, 0);
}
