...
function semanticResolver(): SearchResolver {
  return {
    ...
    async tryResolve(request, context) {
      ...
      const databasePath =
        configPath === undefined ? getDefaultDbPath() : path.join(localDirectory, "index.sqlite");
    ...
    format(payload) {
      ...
      const lines = results.flatMap((result, index) => [
        `${String(index + 1)}. ${result.title} — qmd://${result.displayPath} #${result.docid} (${result.score.toFixed(
          3,
        )})`,
        result.body
          .trim()
