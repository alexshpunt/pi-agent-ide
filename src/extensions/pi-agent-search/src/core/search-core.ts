...
export interface SearchCore
{
    ...
    execute(request: SearchRequest, context: SearchContext): Promise<AgentToolResult<SearchToolDetails>>;
    runAction(
        reference: string,
        capability: string,
        resolverId: string,
...
export function createSearchCore(): SearchCore
{
    ...
    const core: SearchCore = {
        registerPlugin(plugin): Promise<void>
        {
            ...
            const ready = queue.then(async () =>
            {
                ...
                for (const action of draftActions)
                {
                    ...
                    if (
                        actions.has(key)
                        || draftActions.some((candidate) =>
                            candidate !== action && actionKey(candidate.resolverId, candidate.capability) === key
                        )
                    )
                    {
        ...
        async execute(request, context): Promise<AgentToolResult<SearchToolDetails>>
        {
            ...
            const snapshot = [...resolvers].sort((left, right) =>
        ...
        renderPromptGuideline(): string | undefined
        {
            const entries = [...descriptions].flatMap(([id, source]) =>
            {
                ...
                return description === undefined ? [] : [`- \`${id}\` — ${description.replaceAll("\n", "\n  ")}`];
            });
            return entries.length === 0
                ? undefined
                : indentGuidelineContinuation(["Search supports these installed protocols:", ...entries].join("\n"));
        },
        renderer(resolverId)
        {
            return resolvers.find(({ registration }) => registration.resolver.id === resolverId)?.registration.resolver
                .renderResult;
        },
    };
...
function assertResolver(value: unknown): asserts value is SearchResolverRegistration
{
    ...
    if (
        ...
        || typeof resolver.tryResolve !== "function" || typeof resolver.format !== "function"
    )
    {
        throw new TypeError("Invalid search resolver registration");
    }
...
{
    if (
        !isRecord(value) || typeof value.resolverId !== "string" || value.resolverId.trim().length === 0
...
function isAttempt(value: unknown): value is SearchResolutionAttempt
{
    ...
    return kind === "not-handled" || (kind === "resolved" && "payload" in value)
