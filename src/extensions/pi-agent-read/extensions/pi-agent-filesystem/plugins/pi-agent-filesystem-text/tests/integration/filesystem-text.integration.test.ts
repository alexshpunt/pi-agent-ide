...
test("registers text conversion for both filesystem capabilities", async () =>
{
    ...
    await expect(readHost.convert(input, {})).resolves.toEqual([{ type: "text", text: "fixture text" }]);
    await expect(writeHost.convert(input, {})).resolves.toEqual([{ type: "text", text: "fixture text" }]);
});
