...
import type { AgentContent, ContentHost, ContentInput, Resource, ResourceResolver } from "pi-agent-resource";
...
test("keeps read and write capabilities on separate resources", async () =>
{
    await withTempDirectory(async (directory) =>
    {
        ...
        await expect(readAttempt.resource.read?.({})).resolves.toEqual([{ type: "text", text: "read content" }]);
        await expect(writeAttempt.resource.read?.({})).resolves.toEqual([{ type: "text", text: "write content" }]);
        ...
        await expect(writeAttempt.resource.write([
...
test("does not claim explicit non-filesystem references", async () =>
{
    ...
    await expect(resolver.tryResolve("https://example.com/file.txt", { cwd: "/workspace" }))
...
async function resolveReadable(resolver: ResourceResolver, source: string, cwd: string): Promise<Resource>
{
