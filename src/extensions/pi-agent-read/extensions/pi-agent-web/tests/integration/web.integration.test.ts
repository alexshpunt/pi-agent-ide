...
test("recognizes only HTTP sources without performing I/O during resolution", async () => {
  ...
  await expect(
    resolver.tryResolve("ftp://example.test/file", { cwd: "/workspace" }),
...
test("honors caller cancellation and the configured timeout", async () => {
  ...
  const cancellationAttempt = await cancellationResolver.tryResolve(`${baseUrl}/slow`, {
    cwd: "/workspace",
  });
  ...
  await expect(
    cancellationAttempt.resource.read({ signal: controller.signal }),
