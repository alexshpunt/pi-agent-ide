...
export function projectReadState(state: ReadState, request: ReadRequest): ReadToolResult
{
    ...
    const renderedText = state.textMode === "final"
        ? renderFinalTextLines(lines, range.endIndex < totalLines)
        : renderPresentedTextDocument({
            ...state.text,
            lines: lines.map((line, index) => ({
                ...line,
                lineEnding: index === lines.length - 1 && range.endIndex < totalLines ? "" : line.lineEnding,
            })),
        });
...
function renderFinalTextLines(lines: ReadTextState["text"]["lines"], stoppedEarly: boolean): string
{
...
function resolveTextRange(request: ReadRequest, totalLines: number): { startIndex: number; endIndex: number; }
{
    const offset = request.offset === undefined ? 1 : Math.trunc(request.offset);
    const startLine = offset < 0
        ? Math.max(1, totalLines + offset + 1)
        : Math.max(1, offset);
