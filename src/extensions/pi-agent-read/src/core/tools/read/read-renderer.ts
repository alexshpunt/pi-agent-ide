...
function renderContentRows(state: ReadPanelState, width: number): string[] {
  ...
  if (
    ...
    isMarkdownSource(state.details, lines, state.options.kind)
  ) {
...
function renderTopBorder(state: ReadPanelState, width: number, theme: Theme): string {
  ...
  const title = truncateToWidth(
    ` ${readTitle(state, Math.max(1, width - 1))} `,
    Math.max(0, width - 1),
    "",
  );
...
function readTitle(state: ReadPanelState, width: number): string {
  const label =
    state.options.label ?? languageLabel(languageFor(state.details, readLines(state.details)));
...
function cleanText(
  ...
  if (
    ...
    !(
      details.totalLines !== undefined &&
      details.endLine !== undefined &&
      details.endLine < details.totalLines
    )
  ) {
    return text;
  }
...
function normalizeText(value: string): string {
  ...
  for (const character of value.replaceAll("\t", " ".repeat(4))) {
