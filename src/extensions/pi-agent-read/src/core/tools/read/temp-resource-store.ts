...
export class TempResourceStore
{
    ...
    #resolve(source: string): ResourceResolutionAttempt
    {
        ...
        if (entry === undefined)
        {
            return { kind: "failed", error: new Error(`Temporary resource ${source} expired or does not exist`) };
        }
    ...
    async #removeExpired(): Promise<void>
    {
        ...
        const expired = [...this.#entries.entries()].filter(([, entry]) =>
            entry.activeReads === 0 && entry.lastUsedAt + this.#ttlMs <= now
        );

        await Promise.all(expired.map(async ([source, entry]) =>
        {
