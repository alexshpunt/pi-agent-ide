...
import { createReadState, failureResult, projectReadState } from "#src/core/tools/read/read-result.js";
...
export interface ReadTool
{
    ...
    execute(request: ReadPipelineContext["request"], context: ResourceResolverContext): Promise<ReadToolResult>;
...
{
    const tempResources = new TempResourceStore();
    const resolvers: RegisteredResolver[] = [{
        resolver: tempResources.resolver,
    ...
    return {
        tool: {
            ...
            label: toolId,
            description:
                `Resolve a supported source into agent-native content. Text output is truncated to ${READ_OUTPUT_MAX_LINES} lines or ${
            ...
            renderResult(result, options, theme, context)
            {
                ...
                const renderer = resolvedBy === undefined
                    ? undefined
            ...
            async execute(_toolCallId, params, signal, _onUpdate, context)
            {
                ...
                return executeRead(params, resolverContext, resolvers, handlers, presenters, tempResources);
            },
        },
        execute(request, context): Promise<ReadToolResult>
        {
            return executeRead(request, context, resolvers, handlers, presenters, tempResources);
        },
        registerContributions(pluginId, contributions): void
        {
            ...
            for (const registration of incomingResolvers)
            {
                resolvers.push({
                    ...
                    ...(registration.renderResult === undefined ? {} : { renderResult: registration.renderResult }),
...
async function executeRead(
    ...
    tempResources: TempResourceStore,
): Promise<ReadToolResult>
{
    ...
    const projected = projectReadState(pipeline.state!, pipeline.request);
    pipeline = { ...pipeline, result: projected };
    const postRead = await runPostReadHandlers(pipeline, handlerSnapshot);
    const result = postRead.kind === "return" ? postRead.result : postRead.context.result!;
    const saveFullOutput = pipeline.request.limit === undefined && pipeline.state?.preserveTruncatedOutput === true
        ? (text: string): Promise<string> => tempResources.save(text)
        : undefined;
...
{
    const source = initialContext.request.path!;
...
): Promise<ReadStageOutcome>
{
    ...
    const matchingHandlers = initialState === undefined
        ? []
        : handlers.filter((registered) =>
...
): Promise<ReadPipelineContext>
{
    ...
    const contributions = await Promise.all(presenters.map(async ({ registration }) => ({
...
): TextDocument
{
    ...
    for (const contribution of contributions)
    {
        ...
        lines = lines.map((line, index) =>
            mergePresentedLine(base.lines[index]!, line, contribution.document.lines[index]!)
        );
    }
...
{
    const presentation = mergeLinePresentation(base.presentation, current.presentation, contribution.presentation);
...
): readonly PresentedTextRow[] | undefined
{
    ...
    {
        return [...contributionRows.slice(0, contributionRows.length - baseRows.length), ...currentRows];
    }

    return [...currentRows, ...contributionRows];
}

function startsWithRows(rows: readonly PresentedTextRow[], prefix: readonly PresentedTextRow[]): boolean
{
...
function endsWithRows(rows: readonly PresentedTextRow[], suffix: readonly PresentedTextRow[]): boolean
{
    const offset = rows.length - suffix.length;
    return suffix.length > 0
        && offset >= 0
        && suffix.every((row, index) => rows[offset + index] === row);
}
...
{
    return (registration.when.resolvedBy === "any" || registration.when.resolvedBy === state.resolvedBy)
