...
export default async function registerTemporaryResourceFixture(pi: ExtensionAPI): Promise<void> {
  pi.on("tool_call", (event, context) => {
    if (
      event.toolName !== "read" ||
      (event.input as { path?: string }).path !== "temp:fixture-latest"
    ) {
      return;
    }
    ...
    const latest = context.sessionManager.getBranch().findLast((entry) => {
      ...
      return (
        typeof (entry.message.details as { temporarySource?: unknown } | undefined)
          ?.temporarySource === "string"
      );
    });
    const temporarySource =
      latest?.type === "message" && latest.message.role === "toolResult"
