...
export async function generatePluginRegistrationExtensions(): Promise<GeneratedPluginRegistrationExtensions>
{
    ...
    const directory = await mkdtemp(path.join(tempRoot, "pi-agent-read-plugin-registration-"));
...
): string
{
    ...
    const resourceApi =
        pathToFileURL(path.resolve("agent/src/extensions/pi-agent-ide/packages/pi-agent-resource/src/index.ts")).href;
