...
test("routes a restored result through its resolver renderer", async () =>
{
    ...
    const rendered = read.tool.renderResult!(
        result,
...
test("includes resolver and resource Error messages in read failures", async () =>
{
    ...
    resolverRead.registerContributions("resolver-errors", { resolvers: [{ resolver: failedResolver }] });

    const resolverResult = await executeRead(resolverRead, "symbol:file.ts");
    expect(resolverResult.content[0]).toMatchObject({
        type: "text",
        text: expect.stringContaining(resolverCause.message),
    });
    ...
    resourceRead.registerContributions("resource-errors", { resolvers: [{ resolver: resourceResolver }] });

    const resourceResult = await executeRead(resourceRead, "symbol:file.ts#missing");
    expect(resourceResult.content[0]).toMatchObject({
        type: "text",
        text: expect.stringContaining(resourceCause.message),
    });
...
{
    const lineLimited = Array.from({ length: DEFAULT_MAX_LINES + 1 }, (_, index) => `line ${index + 1}`);
...
{
    const sourceLines = Array.from({ length: DEFAULT_MAX_LINES + 1 }, (_, index) => `line ${index + 1}`);
    const read = createReadTool();
    ...
    try
    {
        ...
        expect(first.content[0]).toMatchObject({
            type: "text",
            text: expect.stringContaining(`Full output: ${temporarySource}`),
        });
        ...
        expect(remainder.content).toEqual([{ type: "text", text: `final|line ${DEFAULT_MAX_LINES + 1}` }]);
...
test("does not save an explicit range as a temporary resource", async () =>
{
    ...
    try
    {
        ...
        expect(result.content[0]).toMatchObject({ type: "text", text: expect.not.stringContaining("temp:") });
    }
    finally
    {
        await read.dispose();
...
test("does not return a partial line when the first line exceeds the byte limit", async () =>
{
    ...
    expect(result.content).toEqual([{
        type: "text",
        text:
            "[Line 1 is 50.0KB, exceeds 50.0KB limit. Use a source-specific tool to read this line in smaller byte ranges.]",
    }]);
