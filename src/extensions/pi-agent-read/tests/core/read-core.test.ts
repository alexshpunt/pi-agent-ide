...
import { READ_API_VERSION, READ_PROTOCOL, type ReadPlugin, type ReadPluginApi } from "#src/api/plugin-protocol.js";
...
test("renders static descriptions and evaluates lazy descriptions for each snapshot", async () =>
{
    ...
    await core.registerPlugin(plugin("static", (api) =>
    {
        api.describe("Reads static fixture content.");
    }));
    await core.registerPlugin(plugin("lazy", (api) =>
    {
        api.describe(() =>
        {
            calls += 1;
...
test("fails prompt construction for invalid or throwing lazy descriptions", async () =>
{
    ...
    await invalidCore.registerPlugin(plugin("invalid", (api) =>
    {
    ...
    await throwingCore.registerPlugin(plugin("throwing", (api) =>
    {
        api.describe(() =>
        {
            throw failure;
...
test("rejects a second description without committing the failed setup", async () =>
{
    ...
    await expect(core.registerPlugin(plugin("duplicate", (api) =>
    {
        api.describe("First.");
        api.describe(() => "Second.");
