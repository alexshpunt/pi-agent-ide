...
describe("text change document", () =>
{
    ...
    {
        const document = new TextChangeDocument("one\ntwo\nthree");
        expect(applyTextChanges(document.content, [document.replaceLines(1, 1, "ONE")]).content)
            .toBe("ONE\ntwo\nthree");
        expect(applyTextChanges(document.content, [document.deleteLines(3, 3)]).content)
            .toBe("one\ntwo");
    });
    ...
    test("handles a final newline and an empty document", () =>
    {
        ...
        expect(applyTextChanges(finalNewline.content, [finalNewline.deleteLines(2, 2)]).content).toBe("one\n");

        const empty = new TextChangeDocument("");
        expect(applyTextChanges(empty.content, [empty.replaceAll("first")]).content).toBe("first");
        expect(applyTextChanges(empty.content, [empty.insertAfterLine(1, "first")]).content).toBe("first");
    });

    test("inserts before and after lines with the document line ending", () =>
    {
        const lf = new TextChangeDocument("one\ntwo");
        expect(applyTextChanges(lf.content, [lf.insertBeforeLine(1, "zero")]).content).toBe("zero\none\ntwo");
        expect(applyTextChanges(lf.content, [lf.insertAfterLine(2, "three")]).content).toBe("one\ntwo\nthree");

        const crlf = new TextChangeDocument("one\r\ntwo\r\nthree");
        expect(applyTextChanges(crlf.content, [crlf.replaceLines(2, 2, "TWO")]).content)
            .toBe("one\r\nTWO\r\nthree");
        expect(applyTextChanges(crlf.content, [crlf.insertAfterLine(1, "middle")]).content)
            .toBe("one\r\nmiddle\r\ntwo\r\nthree");
    });
    ...
    test("applies a same-document move as one changeset", () =>
    {
        ...
        const changes = [
            ...
            document.insertAfterLine(4, document.text(range)),
        ];
