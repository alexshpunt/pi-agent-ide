...
function registration(execute: PartialToolRecoveryRegistration["execute"]): PartialToolRecoveryRegistration
{
    return {
        toolName: "replace",
        extractEntries: () => [
...
describe("PartialToolRecoveryRegistry", () =>
{
    ...
    it("skips entries that already completed before recovery", async () =>
    {
        ...
        expect(result?.skippedEntries).toContainEqual({ index: 0, reason: "entry already completed before recovery" });
        expect(execute).toHaveBeenCalledWith(
