...
describe("resolvePartial", () =>
{
    ...
    it("parses a complete JSON with edits array", () =>
    {
        ...
        expect(edits![0]).toMatchObject({ path: "a.ts", start: "begin" });
        expect(edits![1]).toMatchObject({ path: "b.ts", start: "1#ABCD" });
    });
    ...
    {
        const json =
            "{\"edits\":[{\"path\":\"a.ts\",\"start\":\"begin\",\"end\":\"end\",\"text\":\"ok\"},{\"path\":\"b.ts\"";
        const edits = editsOf(json);
        expect(edits).toHaveLength(2);
        expect(edits![0]).toMatchObject({ path: "a.ts" });
        expect(edits![1]).toMatchObject({ path: "b.ts" });
    });
    ...
    it("extracts from writes array", () =>
    {
        ...
        expect(writes![0]).toMatchObject({ path: "a.ts" });
        expect(writes![1]).toMatchObject({ path: "b.ts" });
    });
    ...
    it("extracts from single write entry (partial)", () =>
    {
        ...
        expect(writes![0]).toMatchObject({ path: "a.ts", content: "hello" });
    });
    ...
    it("handles nested arrays inside edits", () =>
    {
        ...
        const edits = editsOf(json);
        expect(edits).toHaveLength(1);
        expect(edits![0]).toMatchObject({ path: "a.ts", tags: ["one", "two", "three"] });
    });
