...
function registration(name = "fixture"): TextMutationToolRegistration<typeof schema>
{
    return {
        ...
        mutate: (context) => ({
            edits: new Map([[
                "fixture",
                { changes: [context.sourceDocument.replaceAll("fixture")], action: "edited" },
            ]]),
        }),
    };
}
...
test("validates mutation metadata against its parameter schema", () =>
{
    ...
    expect(() =>
        ...
        } as unknown as AnyTextMutationToolRegistration)
    ).toThrow("missing");
    expect(() =>
        assertTextMutationToolRegistration({
            ...registration(),
    ...
    expect(() =>
        assertTextMutationToolRegistration({
            ...registration(),
            anchors: [{ field: "start", sourceField: "file", kinds: ["fixture/position", "fixture/position"] }],
        } as unknown as AnyTextMutationToolRegistration)
...
{
    const core = createTextEditorCore();
    await expect(core.registerPlugin({
        protocol: "pi-agent-text-editor",
        apiVersion: 15,
        id: "invalid-plugin",
        setup(api)
