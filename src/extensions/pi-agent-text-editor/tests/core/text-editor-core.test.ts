...
test("runs registered edit handlers around the core operation", async () =>
{
    ...
    const outcome = await core.executeEdit(
        ...
        (state) =>
        {
            ...
            return `${state.input as string}:operation`;
        },
    );
...
test("reads and writes through the same resource", async () =>
{
    ...
    await core.registerPlugin(plugin);
    const outcome = await core.editText(
        "notes.md",
        { cwd: "/workspace" },
        (text) => ({ text: text.replace("before", "after"), result: { changed: true } }),
    );
...
test("validates resource read content and final write content", async () =>
{
    ...
    await expect(invalidReadCore.editText(
        "notes.md",
        { cwd: "/workspace" },
        (text) => ({ text, result: undefined }),
    ...
    await expect(invalidWriteCore.editText(
        "notes.md",
        { cwd: "/workspace" },
        () => ({ text: 42 as never, result: undefined }),
    )).resolves.toMatchObject({
        kind: "failed",
...
test("rejects duplicate resolvers without installing the failed setup draft", async () =>
{
    ...
    await expect(core.editText(
        "notes.md",
        { cwd: "/workspace" },
        (text) => ({ text: `${text}!`, result: undefined }),
