...
test("waits for post-edit work after writing and rereads the final text", async () =>
{
    ...
    await core.registerPlugin(resourcePlugin(mutableResolver(() => text, (next) =>
    {
        text = next;
    })));
...
test("presents the content reread after post-edit work", async () =>
{
    ...
    await core.registerPlugin({
        ...resourcePlugin(mutableResolver(() => text, (next) =>
        {
            text = next;
        })),
        ...
        {
            api.addResolver({
                resolver: mutableResolver(() => text, (next) =>
                {
                    text = next;
                }),
            });
    ...
    await expect(core.editText(
        "notes.md",
        { cwd: "/workspace" },
        () => ({ text: "requested", result: undefined }),
