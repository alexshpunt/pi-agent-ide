...
const packageRoot = path.resolve("agent/src/extensions/pi-agent-ide/extensions/pi-agent-text-editor");
...
export async function generatePluginRegistrationExtensions(): Promise<GeneratedPluginRegistrationExtensions>
{
    ...
    const directory = await mkdtemp(path.join(tempRoot, "plugin-registration-"));
    const coreExtension = path.join(directory, "core-extension.ts");
    const asynchronousCoreFirstPlugin = path.join(directory, "asynchronous-core-first-plugin-extension.ts");
    const asynchronousPluginFirstPlugin = path.join(directory, "asynchronous-plugin-first-plugin-extension.ts");
