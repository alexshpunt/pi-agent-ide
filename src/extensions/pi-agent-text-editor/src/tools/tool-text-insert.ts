...
export const insertSchema = Type.Object({
    ...
    anchor: Type.String({ description: "Registered text anchor; text is inserted after its resolved line" }),
...
export const insertMutationTool: TextMutationToolRegistration<typeof insertSchema> = {
    ...
    mutate: async (context, params: InsertParams) =>
    {
        ...
        {
            const changes = insertionChanges(context, selections, params.text);
            return {
        ...
        return {
            edits: new Map([[source, {
                changes: [context.documentFor(source).insertAfterLine(anchor.lineNumber, params.text)],
                action: "edited",
            }]]),
        };
    },
