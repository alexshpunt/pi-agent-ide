...
import { TextAnchorRegistry, type TextAnchorRegistrySnapshot } from "#src/core/text-anchor-registry.js";
import { applyTextChanges, type TextChange, type TextChangeResult } from "#src/core/text-change-engine.js";
import { previewTextMutation } from "#src/core/text-mutation.js";

import type { TextAnchorInspectionOutcome, TextAnchorInspectionRequest } from "#src/api/anchor-inspection.js";
import type { TextEditCompletion, TextEditCompletionListener, TextEditIntent } from "#src/api/edit-completion.js";
import type {
...
type StageRunOutcome<State> = StageRunCompleted<State> | {
...
export type TextResourcesEditOutcome<Result> =
    | {
        readonly kind: "completed";
        readonly resources: readonly Exclude<TextResourceEditOutcome<unknown>, { readonly kind: "failed"; }>[];
        readonly result: Result;
    }
    | { readonly kind: "failed"; readonly failure: TextResourceEditFailure; readonly completed: readonly string[]; };
...
export interface TextResourcesEditContext extends ResourceResolverContext, TextMutationGuardContext
{}
...
export function createTextEditorCore(
    registerMutationTool?: (registration: AnyTextMutationToolRegistration, core: TextEditorCore) => void,
): TextEditorCore
{
    ...
    const registerContributions = (draft: PluginContributionDraft): void =>
    {
        ...
        for (const contribution of incomingPresenters)
        {
            ...
            {
                throw new Error(`Text presenter ${contribution.registration.presenter.id} is already registered`);
            }
    ...
    const core: TextEditorCore = {
        ...
        {
            const resolverSnapshot = [...resolvers].sort((left, right) =>
                left.priority - right.priority || left.order - right.order
            );
            const presenterSnapshot = [...presenters].sort((left, right) =>
                (left.registration.priority ?? 0) - (right.registration.priority ?? 0) || left.order - right.order
            );
        ...
        previewTexts(sources, context, operation): Promise<TextMutationPreviewOutcome>
        {
            ...
            return previewTextResources(sources, context, resolverSnapshot, anchorRegistry.snapshot(), operation);
        },
        ...
        ): Promise<TextEditExecutionOutcome<Input, Result>>
        {
            ...
            const postEdit = await runResultHandlers("text-post-edit", edit.state, invocationHandlers, tool);
        ...
        renderGeneralPromptGuideline(): string | undefined
        {
            ...
            for (const registeredPlugin of plugins.values())
            {
                ...
                const contribution = writablePromptContributions.find((candidate) =>
                    candidate.pluginId === registeredPlugin.plugin.id
                );
            ...
            {
                sections.push(["Text edits support these writable resources:", ...writableEntries].join("\n"));
            }
        ...
        renderToolPromptGuideline(tool): string | undefined
        {
            ...
            for (const registeredPlugin of plugins.values())
            {
                ...
                const contribution = promptContributions.find((candidate) =>
            ...
            return entries.length === 0
                ? undefined
                : indentGuidelineContinuation([`${tool} supports these installed extensions:`, ...entries].join("\n"));
        },
...
): Promise<TextAnchorInspectionOutcome>
{
    ...
    for (
        const { resolver } of [...resolvers].sort((left, right) =>
            left.priority - right.priority || left.order - right.order
        )
    )
    {
        ...
        {
            return { kind: "failed", reason: `Resolver ${resolver.id} returned an invalid result`, cause: attempt };
        }
        ...
        {
            return { kind: "failed", reason: `Resource ${resource.source} is not readable text`, cause: content };
        }
...
): Promise<TextMutationPreviewOutcome>
{
    try
    {
        ...
        {
            const request = requestBySource.get(source)!;
        ...
        const mutation = await operation(texts, (source, value, kinds) =>
        {
            ...
            return anchorResolvers.resolve(value, item.anchorContext, kinds === undefined ? undefined : new Set(kinds));
        });
...
): Promise<TextResourcesEditOutcome<Result>>
{
    ...
    for (const source of sources)
    {
        ...
        try
        {
            await item.resource.write(finalContent, context.signal === undefined ? {} : { signal: context.signal });
        }
        catch (error)
        {
            return {
...
): Promise<PreparedTextResource | { readonly failure: TextResourceEditFailure; }>
{
    for (const { resolver } of resolvers)
    {
        ...
        if (read)
        {
            ...
            try
            {
                content = await attempt.resource.read!(context.signal === undefined ? {} : { signal: context.signal });
            }
            catch (error)
            {
                if (!allowReadFailure)
...
): PluginContributionController
{
    ...
    const api: TextEditorPluginApi = {
        addMutationTool(registration): void
        {
            ...
            if (state === "setup")
            {
                setupDraft.mutationTools!.push(registration);
                return;
            }
        ...
        addToolRenderer(registration): void
        {
            ...
            if (state === "setup")
            {
                setupDraft.toolRenderers!.push(registration);
                return;
            }
        ...
        addMutationGuard(registration): void
        {
            ...
            if (state === "setup")
            {
                setupDraft.mutationGuards!.push(registration);
                return;
            }
        ...
        onMutationTool(listener): () => void
        {
            ...
            if (state === "setup")
            {
                setupDraft.mutationToolListeners!.push(listener);
                return () =>
                {
                    const index = setupDraft.mutationToolListeners!.indexOf(listener);

                    if (index !== -1)
                    {
                        setupDraft.mutationToolListeners!.splice(index, 1);
                    }
                };
            }
        ...
        onDidEdit(listener): () => void
        {
            ...
            if (state === "setup")
            {
                setupDraft.editCompletionListeners!.push(listener);
                return () =>
                {
                    const index = setupDraft.editCompletionListeners!.indexOf(listener);

                    if (index !== -1)
                    {
                        setupDraft.editCompletionListeners!.splice(index, 1);
                    }
                };
            }
        ...
        addTextPresenter(registration): void
        {
            ...
            if (state === "setup")
            {
                setupDraft.presenters!.push(contribution);
                return;
            }
...
): void
{
    ...
    for (const handler of draft.handlers)
    {
        ...
        {
            throw new TypeError(`Plugin ${handler.pluginId} provided an invalid handler for ${handler.tool}`);
        }
    ...
    for (const contribution of draft.promptContributions)
    {
        ...
        {
            throw new Error(`Plugin ${contribution.pluginId} describes ${contribution.tool} more than once`);
        }
...
{
    if (contributions.some((contribution) => descriptionKey(contribution) === descriptionKey(incoming)))
    {
...
function indentGuidelineContinuation(guideline: string): string
{
    ...
    return [firstLine ?? "", ...continuationLines.map((line) => line.length === 0 ? line : `  ${line}`)].join("\n");
}
...
{
    const merged = registration.fallback === true
...
function addMutationRegistration(
    ...
    registerMutationTool: ((registration: AnyTextMutationToolRegistration, core: TextEditorCore) => void) | undefined,
    core: TextEditorCore,
): void
{
    assertTextMutationToolRegistration(registration);
