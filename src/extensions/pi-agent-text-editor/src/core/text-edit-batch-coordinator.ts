...
export interface ToolBatchDefinition<TState = unknown>
{
    ...
    readonly buildArguments: (calls: readonly BatchCallCandidate<TState>[]) => Record<string, unknown>;
    readonly execute: (
        toolCallId: string,
...
export function registerBlockedToolCall(blocked: BlockedToolCall): void
{
    ...
    if (plan && !plan.blockedCalls.some((item) => item.callId === blocked.callId))
    {
...
{
    if (
        !isObjectNotArray(value) || value.type !== "toolCall" || typeof value.id !== "string"
...
function publishRenderArguments(definition: ToolBatchDefinition, candidate: BatchCallCandidate): void
{
...
{
    if (!isObjectNotArray(message) || message.role !== "assistant" || !Array.isArray(message.content))
    {
        return undefined;
    }
    ...
    for (let index = 0; index < content.length;)
    {
        ...
        const firstCandidate = first && definition
        ...
        while (nextIndex < content.length)
        {
            ...
            const candidate = definition.resolveCall(next, run.at(-1)!.state);
...
function filterProviderTools(state: CoordinatorState, event: { payload?: unknown; }): unknown
{
    ...
    return {
        ...
        tools: payload.tools.filter((tool) =>
        {
            ...
            const functionName = isObjectNotArray(tool.function) && typeof tool.function.name === "string"
                ? tool.function.name
                : undefined;
            ...
            return (toolName === undefined || !syntheticTools.has(toolName))
...
): Promise<BatchExecution>
{
    plan.execution ??= (async () =>
    {
        ...
        try
        {
            const executable = plan.candidates.filter(({ call }) => plan.journal.get(call.id).state !== "blocked");
            const params = plan.definition.buildArguments(executable);
        ...
        await recoverBatch(plan, signal, ctx);
...
async function recoverBatch(plan: BatchPlan, signal: AbortSignal | undefined, ctx: ExtensionContext): Promise<void>
{
...
function finalBatchResults(plan: BatchPlan, executionError: unknown): ReadonlyMap<string, AgentToolResult<unknown>>
{
    return new Map(plan.calls.map((call) =>
    {
...
): Promise<AgentToolResult<TDetails>>
{
    ...
    {
        if (
            plan.definition.splitResult === undefined
            && plan.carrierId !== toolCallId
        )
        {
            return {
        ...
        if (plan.definition.splitResult !== undefined)
        {
            ...
            return markBatchResult(result as AgentToolResult<TDetails>, plan.batchId, batchDisplayCalls(plan));
        }
...
export function registerToolBatch<TState>(pi: ExtensionAPI, definition: ToolBatchDefinition<TState>): void
{
    ...
    const definitionForCoordinator: ToolBatchDefinition = {
        ...
        resolveCall: (call, inheritedState) => definition.resolveCall(call, inheritedState as TState | undefined),
        buildArguments: (calls) => definition.buildArguments(calls as readonly BatchCallCandidate<TState>[]),
