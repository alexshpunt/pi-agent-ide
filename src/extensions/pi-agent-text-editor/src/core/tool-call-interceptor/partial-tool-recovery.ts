...
export interface PartialToolRecoveryRegistration
{
    ...
    readonly execute: (params: Record<string, unknown>, context: RecoveryExecutionContext) => Promise<unknown>;
}
...
export class PartialToolRecoveryRegistry
{
    ...
    recoveryKeyFor(toolName: string): string | undefined
    {
        for (const [key, registration] of this.registrations)
        {
            if (registration.toolName === toolName || registration.toolNames?.includes(toolName) || key === toolName)
            {
                return key;
            }
        }
    ...
    ): Promise<PartialToolRecoveryResult>
    {
        ...
        const complete = candidates.filter((entry) =>
        {
            ...
            {
                skippedEntries.push({ index: entry.index, reason: "entry already completed before recovery" });
                return false;
            }
            ...
            if (
                !input.recoverCompleteBatch && input.staleEntryIndex !== undefined
        ...
        for (const entry of complete)
        {
            ...
            {
                skippedEntries.push({ index: entry.index, reason: "entry was excluded by recovery filtering" });
            }
        }
        ...
        try
        {
            const params = registration.buildParams(valid, input);
            const normalResult = await registration.execute(params, {
                contentIndex: input.contentIndex,
            ...
            return {
                ...base,
                status: "recovered",
                appliedEntries: registration.getAppliedEntries?.(normalResult, valid, input)
