...
const streamingInterceptorState = Symbol.for("pi-agent-text-editor.tool-call-interceptor.streaming-provider.state");
...
): void
{
    ...
    const install = (model: Model<Api> | undefined, ctx: ExtensionContext): void =>
    {
        if (!model)
        {
            return;
        ...
        try
        {
            ...
            for await (const event of upstream)
            {
                ...
                if (interception)
                {
                    ...
                    const interceptedContentIndex = event.type === "toolcall_delta" || event.type === "toolcall_end"
                        ? event.contentIndex
                        : undefined;
                    const partial: AssistantMessage = {
                        ...frozenPartial,
                        content: frozenPartial.content
                            .filter((_block, contentIndex) =>
