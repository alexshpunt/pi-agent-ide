...
export interface ToolCallInterceptorHandler
{
     ...
     */
    intercept(ctx: InterceptorContext): Awaitable<InterceptResult | undefined>;
}
...
export interface ToolCallInterceptor
{
    ...
    processStreamEvent(event: AssistantStreamEvent, ctx: ExtensionContext): Promise<GuardInterception | undefined>;
...
class InterceptorImpl implements ToolCallInterceptor
{
    ...
    private readonly emittedToolCallAnnotationKinds = new Map<number, Set<ToolCallAnnotation["kind"]>>();
    ...
    constructor(private readonly pi: ExtensionAPI)
    {
        ...
        pi.on("message_update", async (event, ctx) =>
        {
            ...
            await this.processStreamEvent(streamEvent, ctx);
        });
    ...
    processStreamEvent(event: AssistantStreamEvent, ctx: ExtensionContext): Promise<GuardInterception | undefined>
    {
        if (event.type === "toolcall_start")
        {
            ...
            if (block?.type === "toolCall")
            {
                ...
                this.installAbortCleanup(event.contentIndex, ctx.signal);
            }
        ...
        if (event.type === "toolcall_delta")
        {
            return this.enqueue(event.contentIndex, async () =>
            {
                ...
                this.installAbortCleanup(event.contentIndex, ctx.signal);
                const partialArgs = resolvePartial(buffer, Allow.OBJ | Allow.ARR) ?? {};
                const completeArgs = parseCompleteArguments(buffer);
                const batchEntryIndex = this.getOrCreateBatchEntryIndex(event.contentIndex);
                this.streamedBatchEntries.set(event.contentIndex, {
                    index: batchEntryIndex,
                    toolName,
                    args: partialArgs,
                    complete: completeArgs !== undefined,
                });

                const handlers = this.handlers.get(toolName);

                if (
                    !toolCallId || !handlers || handlers.length === 0
                    || (!completeArgs && Object.keys(partialArgs).length === 0)
                )
                {
                    return;
                }

                const hasCompleteArgs = completeArgs !== undefined;

                if (hasCompleteArgs && this.completeArgumentContentIndices.has(event.contentIndex))
                {
                    return;
                }
        ...
        if (event.type === "toolcall_end")
        {
            return this.enqueue(event.contentIndex, async () =>
            {
                ...
                {
                    this.completeArgumentContentIndices.add(event.contentIndex);
                    interception = handlers && handlers.length > 0
    ...
    {
        const sameEvent = this.processedStreamEvents.delete(event);
        const key = toolStreamEventKey(event);
    ...
    private rememberOriginalArguments(toolCallId: string, originalArguments: Record<string, unknown>): void
    {
    ...
    ): Promise<GuardInterception | undefined>
    {
        ...
        {
            const result = await handler.intercept(ctx);

            if (result !== undefined)
            {
                this.pendingRecoveryBlocks.push({ blocked: result, ctx });
                ...
                this.blockedCallSink?.({
                    ...
                    arguments: structuredClone(ctx.args),
                    ...(ctx.contentIndex === undefined ? {} : { contentIndex: ctx.contentIndex }),
                    ...(ctx.batchEntryIndex === undefined ? {} : { batchEntryIndex: ctx.batchEntryIndex }),
                ...
                return {
                    ...
                    arguments: structuredClone(ctx.args),
                    result: this.abortToolCall(result, ctx, handler.name, handler.blockExecution === true),
                };
            }
        }
    ...
    {
        const emitted = this.emittedToolCallAnnotationKinds.get(contentIndex) ?? new Set<ToolCallAnnotation["kind"]>();
    ...
    ): AgentToolResult<unknown>
    {
        ...
        if (ctx.contentIndex !== undefined)
        {
            this.abortedContentIndices.add(ctx.contentIndex);
            this.annotateContent(ctx.contentIndex, annotation);
        }
        ...
        this.toolCallRecords.set(ctx.toolCall.id, {
            originalArguments: structuredClone(ctx.args),
            guardResult,
    ...
    private async finalizeRecovery(): Promise<void>
    {
        ...
        for (const group of groups.values())
        {
            ...
            const recovery = Guard.IsObjectNotArray(details) && Guard.IsObjectNotArray(details.recovery)
                ? details.recovery
                : undefined;
    ...
    ): Promise<InterceptResult>
    {
        ...
        const staleEntryIndex = Guard.IsObjectNotArray(details) && typeof details.entryIndex === "number"
            ? details.entryIndex
            : undefined;
        ...
        const recovery = await this.recoveryRegistry.recover({
            ...
            ...(ctx.partialArgs ? { partialArgs: ctx.partialArgs } : {}),
            args: ctx.args,
    ...
    unregister(name: string): void
    {
        for (const [toolName, handlers] of this.handlers)
        {
            this.handlers.set(toolName, handlers.filter((handler) => handler.name !== name));
        }
    }
}
...
function toolStreamEventKey(event: AssistantStreamEvent): string | undefined
{
    if (event.type === "toolcall_end")
    {
        return `${event.type}\u0000${event.contentIndex}\u0000${event.toolCall.id}`;
    }
    ...
    return `${event.type}\u0000${event.contentIndex}\u0000${toolCallId}\u0000${delta}`;
}
...
function freshAnchorsAfterRecovery(recovery: PartialToolRecoveryResult): string[]
{
    ...
    const stalePath = Guard.IsObjectNotArray(recovery.staleDetails) && typeof recovery.staleDetails.path === "string"
    ...
    const matching = stalePath === undefined
        ? candidates
        : candidates.filter((result) => result.path === stalePath);
...
): InterceptResult
{
    ...
    const applied = appliedCount > 0
        ? `${appliedCount} (indices: ${recovery.appliedEntries.join(", ")})`
        : "0";
    const skipped = recovery.skippedEntries.length > 0
    ...
    const blockedDetails = freshAnchors.length > 0 && Guard.IsObjectNotArray(blocked.message.details)
        ? { ...blocked.message.details, freshAnchors }
        : blocked.message.details;
    const blockedContent = freshAnchors.length > 0
        ? replaceCurrentAnchors(blocked.message.content, freshAnchors)
        : blocked.message.content;
    const displayedRecovery = freshAnchors.length > 0 && Guard.IsObjectNotArray(recovery.staleDetails)
...
export function registerToolCallInterceptor(pi: ExtensionAPI, handler: ToolCallInterceptorHandler): void
{
...
{
    return coordinatorFor(pi).processStreamEvent(event, ctx);
}

export function markToolCallStreamEventProcessed(pi: ExtensionAPI, event: AssistantStreamEvent): void
{
    coordinatorFor(pi).markStreamEventProcessed(event);
}

export function getToolCallRecord(pi: ExtensionAPI, toolCallId: string): GuardCallRecord | undefined
{
