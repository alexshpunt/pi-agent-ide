    ...
    defineTool<TParameters, FileMutationBatchResult, unknown>({
      ...
      prepareArguments: (arguments_) =>
        ...
        prepareGuardedArguments(
          ...
          definition.anchors?.map(({ field }) => field) ?? [],
        ),
...
> {
  ...
  const hasExplicitAnchor = anchorFields.some((field) => {
...
async function resolveMutationSources(
  ...
  for (const descriptor of definition.anchors ?? []) {
    ...
    if (previous !== undefined && !sameResources(previous, expanded, context.cwd)) {
      throw new Error(
        `Mutation anchors for ${descriptor.sourceField} refer to different resources.`,
      );
    }
  ...
  for (const descriptor of sourceDescriptors) {
    ...
    if (expanded !== undefined) {
      if (explicitSource === undefined) {
        fields.set(descriptor.field, requiredValue(expanded[0]));
      } else {
        ...
        if (!sameResource(explicitSource, requiredValue(expanded[0]), context.cwd)) {
        ...
        await Promise.all(
          anchorSources.map(async (source) => {
            try {
              return [
                source,
                await resolveResourceAnchor(source, value, descriptor.kinds),
              ] as const;
            } catch (error) {
              throw contextualizeTextMutationAnchorError(
                error,
...
async function buildToolResult(
  ...
  const results = outcome.resources.flatMap((resource) => {
    ...
    return edit === undefined
      ? []
      : [buildSuccessfulTextMutationResult(resource, resultSource, edit)];
  });
...
async function anchorFailureToolResult(
  ...
  const read = await readTextAnchorRecovery(
    core,
