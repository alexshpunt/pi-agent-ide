...
export function formatHint(hint: DiagnosticHint, options: { includeAnchor?: boolean; } = {}): string
{
    const source = hint.source ? `${hint.source}:` : "";
    const anchor = options.includeAnchor === false || hint.anchor === undefined ? "" : `${hint.anchor} — `;
