...
export function renderAgentDiff(fmr: FileMutationResult, diffPath: string): string[]
{
    ...
    const syntaxHints = fmr.hints.filter((hint) => hint.source === "compiler" && hint.severity === "error");
    ...
    for (let rangeIndex = 0; rangeIndex < allRanges.length; rangeIndex += 1)
    {
        ...
        for (const line of diffLines)
        {
            ...
            {
                const presentedLine = presentedAfter.lines[line.afterLine! - 1];
            ...
            const lineNumber = line.afterLine!;
            const metadata = metadataForLine(
                lineNumber,
                fmr.hints,
                visibleRanges,
...
): LineRange[]
{
    ...
    if (rawChanges.length > 0)
    {
        return rawChanges.map((change) => ({
            ...
            end: lineAtOffset(after, Math.max(change.fromB, change.toB - (change.toB > change.fromB ? 1 : 0))),
        }));
    }
...
function expandAndMerge(ranges: readonly LineRange[], lineCount: number): LineRange[]
{
    ...
    const merged: LineRange[] = [windows[0]!];
...
function selectSyntaxHints(hints: readonly DiagnosticHint[], visibleRanges: readonly LineRange[]): DiagnosticHint[]
{
    const visible = hints
        .filter((hint) => isLineVisible(hint.line, visibleRanges))
    ...
    return [
        ...visible,
        ...hidden.slice(0, Math.max(0, MAX_SYNTAX_ERRORS - visible.length)),
    ];
}
...
): string[]
{
    ...
    const lineIsVisible = isLineVisible(lineNumber, visibleRanges);
    const lineHints = hints.filter((hint) =>
        hint.line === lineNumber && (lineIsVisible || allowedSyntaxHints.has(hint))
    );
