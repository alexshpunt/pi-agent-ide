...
export class FileMutationAgentResult
{
    ...
    public toText(): string
    {
        ...
        for (const fmr of this.results)
        {
            parts.push(this.formatSingle(fmr));
        }
    ...
    private formatFailure(fmr: FileMutationResult): string
    {
        ...
        if (
            fmr.errors.some((e) => e.code === "STALE_ANCHOR") && fmr.staleEdits.length > 0
            && !fmr.staleDiagnostic
        )
        {
        ...
        // Header
        lines.push(`EDIT_FAILED (${fmr.errors.length} error${fmr.errors.length === 1 ? "" : "s"})`);
        lines.push("");
        ...
        if (fmr.editRange)
        {
            const endStr = fmr.editRange.end.anchor === "?" ? fmr.editRange.start.anchor : fmr.editRange.end.anchor;
            lines.push(`Requested range: ${fmr.editRange.start.anchor} .. ${endStr}`);
        }
        ...
        if (fmr.staleDiagnostic)
        {
            ...
            if (sd.currentContext.length > 0)
            {
                ...
                for (const ctx of sd.currentContext)
                {
                    const marker = ctx.lineNumber === sd.lineNumber ? ">>" : "  ";
                    lines.push(`  ${marker} ${ctx.lineNumber}#${ctx.hash}  ${ctx.text}`);
                }
            }
        }
        ...
        const mutationRejected = fmr.errors.some((error) => error.code === "MUTATION_REJECTED");
        ...
        if (fmr.hints.length > 0)
        {
            lines.push("");
            lines.push("---");
            lines.push("LSP diagnostics detected. You MUST fix all LSP issues before the result is accepted.");
        }
    ...
    private formatStaleFailure(fmr: FileMutationResult): string
    {
        ...
        if (fmr.staleEdits.length > 0)
        {
            ...
            {
                lines.push(
                    `edits[${s.index}]: STALE_ANCHOR (${s.failurePoint})`,
                );
            }
