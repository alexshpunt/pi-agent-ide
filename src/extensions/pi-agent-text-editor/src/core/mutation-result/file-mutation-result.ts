...
export class FileMutationResult {
  ...
  get editRange(): FileRange | undefined {
    ...
    if (firstEdit?.range) {
      ...
      const lastRange = requiredValue(edits).at(-1)?.range;
  ...
  getDiffFor(path: string): string | undefined {
    ...
    for (const diff of this.diffs) {
      ...
      if (match && (normalisePath(requiredValue(match[1])) === normalised || match[1] === path)) {
        return diff;
      }
    }
  ...
  /**
    ...
    */
  static fromFailureOptions(options: EditFailureOptions): FileMutationResult {
    return new FileMutationResult({
      ok: false,
      path: options.path,
      errors: [{ path: options.path, code: options.code, reason: options.reason }],
      failurePoint: options.failurePoint,
      recovery: options.recovery,
      expectedAnchor: options.expectedAnchor,
      currentAnchor: options.currentAnchor,
      freshAnchors: options.freshLines ? [...options.freshLines] : undefined,
      internalErrorType:
        options.internalError instanceof Error ? options.internalError.name : undefined,
      internalErrorMessage:
        options.internalError instanceof Error ? options.internalError.message : undefined,
    });
  }
  ...
  static ensure(value: unknown): FileMutationResult {
    ...
    if (value && typeof value === "object") {
      ...
      if (object.data && typeof object.data === "object") {
        Object.setPrototypeOf(object, FileMutationResult.prototype);
        return restoreFileMutationResult(object);
      }

      // Raw FileMutationData — construct fresh
      return new FileMutationResult(object);
    }
...
function restoreFileMutationResult(value: unknown): FileMutationResult {
  return value as FileMutationResult;
}
