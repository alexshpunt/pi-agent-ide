...
export function formatStaleAnchorMessage(details: StaleAnchorMessageDetails, reason: string): string
{
    const context = details.context === undefined ? "" : `\n\n${details.context}`;
    return `[SYSTEM] ${details.toolName} blocked: ${details.field} anchor "${details.anchor}" is stale. `
