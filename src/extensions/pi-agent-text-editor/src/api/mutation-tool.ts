...
export interface TextMutationToolRegistration<TParams extends TSchema = TSchema>
{
...
export function assertTextMutationToolRegistration(value: AnyTextMutationToolRegistration): void
{
    ...
    for (const anchor of value.anchors ?? [])
    {
        ...
        {
            throw new TypeError(`Mutation tool ${value.name} anchor ${anchor.field} must accept non-empty kinds`);
        }

        if (new Set(anchor.kinds).size !== anchor.kinds.length)
        {
            throw new TypeError(`Mutation tool ${value.name} anchor ${anchor.field} declares duplicate kinds`);
        }

        if (anchorFields.has(anchor.field))
        {
            throw new TypeError(`Mutation tool ${value.name} declares anchor ${anchor.field} more than once`);
        }
...
{
    const value = params[registration.source.field];
...
function assertProperty(properties: Readonly<Record<string, unknown>>, field: string, label: string): void
{
