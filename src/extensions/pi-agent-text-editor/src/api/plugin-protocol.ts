...
import type { TextMutationToolListener, TextMutationToolRegistration } from "#src/api/mutation-tool.js";
...
const textAnchorResourceResolverSchema = Type.Object({ tryResolve: functionSchema }, { additionalProperties: true });
const textAnchorResourceResolutionAttemptSchema = Type.Union([
    ...
    Type.Object({
        ...
        rejection: Type.Object({
            ...
            reason: Type.String(),
            contextRange: Type.Optional(Type.Object({
                offset: Type.Number(),
                limit: Type.Number(),
            })),
        }),
...
export function isResourceResolverRegistration(value: unknown): value is ResourceResolverRegistration
{
    ...
    return isResourceResolver(registration.resolver)
        && (!("priority" in registration) || typeof registration.priority === "number");
}

export function isTextAnchorResolverRegistration(value: unknown): value is TextAnchorResolverRegistration
{
    ...
    const registration = value as Record<PropertyKey, unknown>;
    return isTextAnchorResolver(registration.resolver)
