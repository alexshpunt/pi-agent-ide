...
export class TypingInterpolation
{
    ...
    public observe(text: string, now: number): void
    {
        ...
        const extendsPrevious = commonPrefixLength(this.observedCharacters, characters)
            === this.observedCharacters.length;
    ...
    public setTarget(text: string, now: number): boolean
    {
        ...
        const visibleChanged = shared < this.visibleCount;
    ...
    private estimatedRate(now: number): number
    {
        ...
        if (first === undefined || last === undefined || last.at - first.at < 20 || last.length <= first.length)
        {
            return DEFAULT_RATE;
        }
    ...
    private pruneArrivals(now: number): void
    {
        ...
        while (this.arrivals.length > 2 && this.arrivals[1]!.at < cutoff)
        {
            this.arrivals.shift();
        }
    }
