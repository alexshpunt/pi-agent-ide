import { createDiffModel, DIFF_CONTEXT_LINES, type DiffModel, type DiffRow, type DiffTextRange } from "./diff-model.js";
...
{
    return resources.map((resource) => projectTypingResource(resource, generatedText, visibleGeneratedText));
}
...
): MutationRenderResource | undefined
{
    ...
    const fullFile = beforeRange !== undefined && isFullFileTyping(resource, beforeRange, range);
    ...
    const rowIndex = model.rows.findLastIndex((row) => row.afterLine === cursorLine && row.kind !== "removed");
    ...
    const expectedText = resource.afterContent.slice(lineStart(resource.afterContent, cursorOffset), cursorOffset);
    ...
    if (
        (!fullFile && (/\s/u.test(appended) || row.text.trimEnd() !== row.text))
        || (fullFile && atLineEnd(resource.afterContent, cursorOffset))
    )
    {
        return undefined;
    }
...
): readonly TextMutationPreviewResource[] | undefined
{
    ...
    for (const resource of resources)
    {
        ...
        extended.push({
            ...resource,
            afterContent: resource.afterContent.slice(0, generatedEnd)
                + appended
...
): MutationRenderResource
{
    ...
    const afterRange = resource.ranges[0]!;
...
{
    const visibleReplacement = resource.afterContent.slice(afterRange.from, cursorOffset)
    ...
    const afterWindow = resource.beforeContent.slice(windowFrom, beforeRange.from)
        + visibleReplacement
...
{
    return beforeRange.from === 0
        && beforeRange.to === resource.beforeContent.length
        && afterRange.from === 0
        && afterRange.to === resource.afterContent.length;
}

function createFullFileTypingModel(beforeContent: string, visibleContent: string, cursorOffset: number): DiffModel
{
    ...
    const rows: DiffRow[] = visibleLines.map((text, index) =>
    {
        ...
        const trailingCursorRow = line === cursorLine
            && index === visibleLines.length - 1
            && text.length === 0
            && visibleContent.endsWith("\n");

        if (beforeText?.trim() === text.trim() || trailingCursorRow)
        {
            return {
                kind: "context",
                text,
...
{
    const row = rows.findLastIndex((candidate) =>
        candidate.afterLine !== undefined && candidate.afterLine <= cursorLine
    );
