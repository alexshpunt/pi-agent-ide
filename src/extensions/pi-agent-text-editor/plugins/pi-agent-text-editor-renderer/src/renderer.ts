...
): TextEditorToolRendererRegistration
{
    ...
    return {
        ...
        renderCall(args, theme, context)
        {
            ...
            const argsComplete = context.argsComplete;
            const key = `${argsComplete ? "complete" : "typing"}:${JSON.stringify(input)}`;
        ...
        renderResult(result, options, theme, context)
        {
            ...
            if (!context.isError && resources.length > 0)
            {
                ...
                const canFinishTyping = typing !== undefined
...
function advancePreviewTarget(state: RenderState, request: PreviewRequest, now: number): boolean
{
    ...
    const resources = extendTypingPreviewResources(target.resources, target.generated, request.generated);
...
): void
{
    ...
    const typing = state.typing;
    const rewound = typing?.setTarget(targetRequest.generated, now) ?? false;
    ensureTypingTimer(state);

    if ((rewound || typing?.hasVisibleText === true) && updateTypingPanel(state))
    {
...
function showPreviewFailure(state: RenderState, reason: string): void
{
    ...
    {
        state.panel.setHeader(renderHeader(state.registration, state.input ?? {}, preview, state.theme));
    }
...
function ensureTypingTimer(state: RenderState): void
{
    ...
    state.timer = setInterval(() =>
    {
        ...
        {
            changed = updateTypingPanel(state);
        }
        ...
        {
            changed = finishPendingResult(state) || changed;
        }
...
function updateTypingPanel(state: RenderState): boolean
{
    ...
    if (target === undefined || typing === undefined || component === undefined || theme === undefined)
    {
        return false;
    }
    ...
    const advanced = state.displayedResources === undefined || state.displayedText === undefined
        ? undefined
        : advanceTypingProjectionResources(
            target.resources,
            target.generated,
    ...
    const resources = advanced ?? projectTypingResources(target.resources, target.generated, visibleText);
    ...
    component.setHeader(renderHeader(state.registration, state.input ?? target.input, preview, theme));
    return true;
}
...
{
    return tool === "write" ? "content" : tool === "insert" || tool === "replace" ? "text" : undefined;
}
...
): string
{
    ...
    const displayedPath = path
        ?? (previewResources.length === 1
            ? previewResources[0]!.path
            : previewResources.length > 1
            ? `${String(previewResources.length)} files`
            : "");
    const sourceLink = path === undefined && previewResources.length === 1
        ? previewResources[0]!.link
        : resourceLink(preview, path);
    const targets = (source.targets ?? [])
        .map(({ field }) => ({ field, path: stringValue(input[field]) }))
        .filter((target): target is { readonly field: string; readonly path: string; } =>
            target.path !== undefined && target.path !== path
        );
    const sourceFallback = targets.length === 0 ? 0 : undefined;
    let header = `${theme.fg("toolTitle", theme.bold(registration.name))} ${
        renderPath(displayedPath, sourceLink, theme)
    }${renderAnchorRange(registration, source.field, input, theme)}${
        renderStats(preview, path, theme, sourceFallback)
    }`;
...
{
    const anchors = (registration.anchors ?? []).filter((anchor) => anchor.sourceField === sourceField);
    const fields = sourceField === registration.source.field && registration.pair !== undefined
        ? registration.pair
...
function resourceLink(preview: MutationRenderOutcome | undefined, path: string | undefined): string | undefined
{
...
): string
{
    ...
    const resource =
        preview.resources.find((candidate) => candidate.path === path || candidate.path.endsWith(`/${path}`))
    ...
    const model = resource.model ?? createDiffModel(resource.beforeContent, resource.afterContent, resource.ranges);
...
{
    return ` ${theme.fg("success", `+${counts.added}`)} ${theme.fg("warning", `~${counts.modified}`)} ${
...
function renderPath(path: string, link: string | undefined, theme: Theme): string
{
    ...
    return link === undefined || link.includes("\u0007") || link.includes("\u001B")
        ? label
        : `\u001B]8;;${link}\u0007${label}\u001B]8;;\u0007`;
}
