...
): DiffModel
{
    ...
    const latestAfterLine = options.focusAfterLine ?? (ranges.length === 0
        ? undefined
        : lineAtOffset(afterContent, ranges.at(-1)!.to) + (options.afterLineOffset ?? 0));
...
{
    const rows: DiffRow[] = [];
    const parts = lineDiff.diff(beforeContent, afterContent, {
    ...
    for (let index = 0; index < parts.length;)
    {
        ...
        if (!part.added && !part.removed)
        {
            ...
            {
                rows.push({ kind: "context", text, beforeLine: beforeLine++, afterLine: afterLine++, changed: false });
            }
...
function alignChangedLines(removed: readonly NumberedLine[], added: readonly NumberedLine[]): readonly DiffRow[]
{
    if (removed.length === 1 && added.length === 1)
    {
        return [modifiedRow(removed[0]!, added[0]!)];
    }
    ...
    for (let oldIndex = 1; oldIndex <= removed.length; oldIndex++)
    {
        ...
        {
            const similarity = lineSimilarity(removed[oldIndex - 1]!.text, added[newIndex - 1]!.text);
            const pair = similarity >= LINE_PAIR_THRESHOLD
                ? scores[oldIndex - 1]![newIndex - 1]! + similarity
    ...
    {
        const similarity = oldIndex > 0 && newIndex > 0
            ? lineSimilarity(removed[oldIndex - 1]!.text, added[newIndex - 1]!.text)
            : 0;
        const pairScore = oldIndex > 0 && newIndex > 0 && similarity >= LINE_PAIR_THRESHOLD
            ? scores[oldIndex - 1]![newIndex - 1]! + similarity
            : Number.NEGATIVE_INFINITY;

        if (pairScore === scores[oldIndex]![newIndex])
        {
            reversed.push(modifiedRow(removed[oldIndex - 1]!, added[newIndex - 1]!));
            oldIndex--;
            newIndex--;
        }
        else if (
            newIndex > 0
            && scores[oldIndex]![newIndex - 1]!
                >= (oldIndex > 0 ? scores[oldIndex - 1]![newIndex]! : Number.NEGATIVE_INFINITY)
        )
        {
            const line = added[newIndex - 1]!;
            reversed.push({ kind: "added", text: line.text, afterLine: line.line, changed: true });
            newIndex--;
        }
        else
        {
            const line = removed[oldIndex - 1]!;
...
function changedTextRanges(before: string, after: string): readonly DiffTextRange[]
{
    ...
    const parts = diffWordsWithSpace(before, after, { timeout: 50, maxEditLength: 5_000 }) ?? diffChars(before, after);
    let offset = 0;
...
{
    return (line.toLowerCase().match(/[\p{L}\p{N}_$]+/gu) ?? [])
        .filter((token) => !NON_SEMANTIC_TOKENS.has(token));
}
...
function directionalTokenSimilarity(source: readonly string[], target: readonly string[]): number
{
    ...
    return source.reduce(
        (total, token) => total + Math.max(...target.map((candidate) => characterSimilarity(token, candidate))),
        0,
    ) / source.length;
}
...
function characterSimilarity(before: string, after: string): number
{
    ...
    const unchanged = diffChars(before, after, { timeout: 25, maxEditLength: 2_000 })
...
function projectHunks(rows: readonly DiffRow[]): readonly DiffRow[]
{
    ...
    for (let index = 0; index < rows.length; index++)
    {
        ...
        while (index + 1 < rows.length && rows[index + 1]!.changed)
        {
            index++;
        }
...
function findFocusRow(rows: readonly DiffRow[], latestAfterLine: number | undefined): number
{
    ...
    {
        const exact = rows.findLastIndex((row) =>
            row.afterLine !== undefined && row.afterLine <= latestAfterLine && row.changed
        );
    ...
    return Math.max(0, rows.findLastIndex((row) => row.changed));
}
