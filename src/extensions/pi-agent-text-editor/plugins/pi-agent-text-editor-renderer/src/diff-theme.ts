...
): DiffThemeTone
{
    ...
    const dark = baseLab.lightness < DARK_BACKGROUND_THRESHOLD;
    ...
    const foreground = deriveForeground(sourceLab, renderedBackground, dark);
...
function deriveForeground(source: Oklab, background: Rgb, dark: boolean): Rgb
{
    ...
    let lightness = dark
        ? clamp(source.lightness, 0.67, 0.86)
        : clamp(source.lightness, 0.34, 0.62);
    let foreground = oklchToRgb(lightness, chroma, hue);

    for (let attempt = 0; attempt < 24 && contrast(foreground, background) < MIN_FOREGROUND_CONTRAST; attempt++)
    {
...
{
    const sequence = ansi.startsWith("\u001B[") ? ansi.slice(2) : ansi;
...
function ansi256ToRgb(index: number): Rgb
{
    ...
    if (index < 232)
    {
        const cube = index - 16;
        return {
            red: CUBE_VALUES[Math.floor(cube / 36)]!,
            green: CUBE_VALUES[Math.floor((cube % 36) / 6)]!,
            blue: CUBE_VALUES[cube % 6]!,
        };
    }
...
): string
{
    return mode === "256color"
        ? `\u001B[${layer};5;${rgbTo256(color, reserved, contrastAgainst)}m`
        : `\u001B[${layer};2;${color.red};${color.green};${color.blue}m`;
}
...
function rgbTo256(color: Rgb, reserved: ReadonlySet<number>, contrastAgainst?: Rgb): number
{
    ...
    {
        const candidateColor = ANSI_256_RGB[index]!;
        ...
        const candidate = ANSI_256_OKLAB[index]!;
        ...
        const distance = lightnessDistance ** 2
            + ANSI_CHROMA_WEIGHT * (aDistance ** 2 + bDistance ** 2);
...
{
    return 0.2126 * srgbToLinear(color.red / 255)
