...
export class MutationPanel implements Component
{
    ...
    public render(width: number): string[]
    {
        ...
        const panels = renderPanels(
            this.diffPanels,
            (resource) => renderDiffPanel(resource, width, this.theme, this.expanded, this.mode === "preview"),
        );
        const ellipsis = `${this.theme.getBgAnsi(this.background)}...`;
        const lines = this.header.length === 0 ? panels : [truncateToWidth(this.header, width, ellipsis), ...panels];
        this.cache = { width, expanded: this.expanded, header: this.header, lines };
        return lines;
    }
}

function renderPanels<Resource>(resources: readonly Resource[], render: (resource: Resource) => string[]): string[]
{
...
): ReadonlyMap<DiffRow, string>
{
    ...
    for (const row of model.rows)
    {
        ...
        const startsNewGroup = line === undefined
            || (previous !== undefined
                && (isBeforeRow(previous) !== isBeforeRow(row)
                    || previousLine === undefined
                    || line !== previousLine + 1));

        if (startsNewGroup && group.length > 0)
        {
            groups.push(group);
            group = [];
        }
    ...
    {
        const normalized = rows.map(({ text }) => text.replaceAll("\t", "    "));
...
function cursorPosition(content: string, offset: number): { readonly line: number; readonly column: number; }
{
