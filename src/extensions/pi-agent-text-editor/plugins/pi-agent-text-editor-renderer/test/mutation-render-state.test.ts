...
import { advanceTypingProjectionResources, projectTypingResources } from "#src/mutation-projection.js";
...
describe("text mutation render state", () =>
{
    test("renders only written replacement lines and keeps the cursor before the implicit line break", () =>
    {
        ...
        const afterContent = beforeContent.slice(0, from) + inserted + beforeContent.slice(from + selected.length);
    ...
    test("treats completed full-file typing lines with edge-only whitespace changes as context", () =>
    {
        ...
        const previousTyping = projectTypingResources([changedResource], changedAfterContent, previousVisible);
    ...
    {
        const beforeLines = Array.from({ length: 30 }, (_, index) => `const value${index + 1} = ${index + 1};`);
        ...
        const preview: TextMutationPreviewResource = {
            path: "stable.ts",
            beforeRanges: [{
                from: beforeContent.indexOf(beforeLines[19]!),
                to: beforeContent.indexOf(beforeLines[19]!) + beforeLines[19]!.length,
            }],
        ...
        expect(model.rows.map(({ afterLine }) => afterLine).filter(Boolean)).toEqual([18, 19, 20, 21, 22]);
