...
describe("diff theme palette", () => {
  ...
  test("reverses background contrast for a light theme and respects ANSI-256 mode", () => {
    ...
    for (const tone of [truecolor.added, truecolor.removed, truecolor.modified]) {
      ...
      expect(contrast(ansiRgb(tone.foreground), ansiRgb(tone.background))).toBeGreaterThanOrEqual(
        3,
      );
    }

    expect(luminance(ansiRgb(truecolor.modified.emphasisBackground))).toBeLessThan(
      luminance(ansiRgb(truecolor.modified.background)),
    );
    ...
    expect(
      new Set([indexed.added.background, indexed.removed.background, indexed.modified.background])
        .size,
    ).toBe(3);
    ...
    expect(Math.min(indexedModified.red, indexedModified.green)).toBeGreaterThan(
      indexedModified.blue,
    );
    ...
    expect(saturation(ansiRgb(indexed.modified.emphasisBackground))).toBeGreaterThan(
      saturation(indexedModified),
    );
