...
describe("semantic text mutation diff", () =>
{
    test("renders a replacement once as the final modified line", () =>
    {
        ...
        expect(model.rows.filter(({ kind }) => kind === "context").map(({ afterLine }) => afterLine)).toEqual([
            2,
            3,
            5,
            6,
    ...
    test("highlights only the new fragment with a stronger theme-derived background", () =>
    {
        ...
        expect(rendered)
            .toContain(`${palette.modified.emphasisBackground}5_000${palette.modified.background}`);
    ...
    test("keeps two context lines around distant hunks and reports the omitted gap", () =>
    {
        ...
        expect(model.rows.map(({ kind, afterLine, omitted }) => ({ kind, afterLine, omitted }))).toEqual([
    ...
    test("merges overlapping context windows without duplicating unchanged lines", () =>
    {
        ...
        expect(model.rows.map(({ afterLine }) => afterLine).filter((line): line is number => line !== undefined))
            .toEqual([
                3,
                4,
                5,
    ...
    test("compact mode clips only the hunk projection while expanded mode keeps all projected hunks", () =>
    {
        ...
        const model = createDiffModel(before, after, [{ from: latest, to: latest + "latest change".length }]);
