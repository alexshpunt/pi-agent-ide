...
function makeBlockResult(toolName: string, violation: ArgumentOrderViolation): InterceptResult {
  ...
  return {
    ...
    message: {
      ...
      content:
        `[SYSTEM] ${toolName} blocked: argument order violation.${unknown}\n` +
...
export function registerArgumentOrderGuard(
  ...
  const handler: ToolCallInterceptorHandler = {
    ...
    blockExecution: true,
    toolNames,
    intercept(context: InterceptorContext): InterceptResult | undefined {
      ...
      const violation = checkArgumentOrder(context.partialArgs ?? context.args, schema);
      return violation ? makeBlockResult(context.toolCall.name, violation) : undefined;
    },
  };
