...
function createStreamingGuard(
  ...
  const handler: ToolCallInterceptorHandler = {
    ...
    async intercept(context) {
      ...
      const input = isEarly
        ? preparePreviewArguments(registration, requiredValue(context.partialArgs))
        : context.args;
  ...
  return plan.resources
    .filter(
      (resource) =>
        ...
        resource.changes.some(
          ({ fromBefore, toBefore }) =>
            fromBefore === 0 && toBefore === resource.before.content.length,
        ),
    )
    .map((resource) => ({
      source: canonicalSource(resource.source, cwd),
      afterContent: resource.after.content,
    }));
}
...
function previewCandidates(
  ...
  const isWritePathIsEnough = early && toolName === "write";
  return resources
    .filter(
      (resource) =>
        ...
        (isWritePathIsEnough ||
          (resource.afterContent !== resource.beforeContent &&
            resource.beforeRanges?.some(
              ({ from, to }) => from === 0 && to === resource.beforeContent.length,
            ) === true)),
    )
    .map((resource) => ({
      source: canonicalSource(resource.path, cwd),
...
function attemptKey(resources: readonly OverwriteCandidate[]): string {
  return [...resources]
    ...
    .map(({ source, afterContent }) => `${source}\u{0}${afterContent}`)
    .join("\u{1}");
}
...
function rejected(): FinalOverwriteDecision {
  const message =
    "The file already exists — a full rewrite may be unnecessary. " +
