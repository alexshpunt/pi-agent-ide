import type { ActiveToolchain, Compiler, Formatter, IdeTool, Linter, ToolContext } from "./types.js";
...
function assertUniqueName(kind: string, list: readonly { name: string; }[], name: string): void
{
    ...
    {
        throw new Error(
            `[pi-agent-ide] ${kind} "${name}" is already registered. Use a unique name.`,
        );
    }
}
...
export function registerTools(tools: readonly IdeTool[]): void
{
    ...
    for (const tool of tools)
    {
        ...
        {
            throw new Error(`[pi-agent-ide] ${tool.kind} "${tool.name}" is registered twice by one plugin.`);
        }
...
{
    const toInfo = (t: { name: string; priority: number; extensions: readonly string[]; }): RegisteredToolInfo => ({
        name: t.name,
        priority: t.priority,
...
async function filterActive<T extends DetectableTool>(tools: readonly T[], ctx: ToolContext): Promise<T[]>
{
    const active: T[] = [];
...
function assertNoPriorityConflicts(active: readonly DetectableTool[]): void
{
    ...
    for (const tool of active)
    {
        ...
        {
            const key = `${ext}@${tool.priority}`;
            const prev = seen.get(key);

            if (prev !== undefined)
            {
                throw new Error(
                    `[pi-agent-ide] priority conflict: tools "${prev}" and "${tool.name}" both claim "${ext}" at priority ${tool.priority}.`,
                );
            }
...
export async function warmup(ctx: ToolContext): Promise<ActiveToolchain>
{
    const [fmts, comps, lnts] = await Promise.all([
        filterActive(formatters, ctx),
        filterActive(compilers, ctx),
        filterActive(linters, ctx),
    ]);

    activeToolchain = { ctx, formatters: fmts, compilers: comps, linters: lnts };
