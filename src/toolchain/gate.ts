...
function matches(tool: { extensions: readonly string[]; }, ext: string): boolean
{
    return tool.extensions.includes(ext) || tool.extensions.includes("*");
}
...
export class Gate
{
    ...
    async runDiagnostics(filePaths: string[]): Promise<Map<string, Diagnostic[]>>
    {
        ...
        const ctx = this.toolchain.ctx;

        for (const filePath of filePaths)
        {
            const ext = extensionOf(filePath);
            const compile = await this.runCompile(filePath, ext, ctx);
            ...
            const lint = await this.runLint(filePath, ext, ctx, false);
    ...
    async runAll(filePaths: string[], options?: { onPhase?: GatePhaseCallback; }): Promise<GateResult>
    {
        const ctx = this.toolchain.ctx;
        ...
        const exts = new Set(filePaths.map((fp) => extensionOf(fp)));
        const compiler = exts.size > 0
            ? this.toolchain.compilers.find((c) => [...exts].some((e) => matches(c, e)))
            : undefined;
        const linter = exts.size > 0
            ? this.toolchain.linters.find((l) => [...exts].some((e) => matches(l, e)))
            : undefined;
        const formatter = exts.size > 0
            ? this.toolchain.formatters.find((f) => [...exts].some((e) => matches(f, e)))
            : undefined;
        ...
        for (const fp of filePaths)
        {
            ...
            const result = await this.runCompile(fp, extensionOf(fp), ctx);
        ...
        const anySyntaxErrors = [...compileResults.values()].some((r) => r.syntaxErrors.length > 0);
        ...
        if (anySyntaxErrors)
        {
            const files: FileGateResult[] = filePaths.map((fp) => ({
                ...
                compile: compileResults.get(fp)
                    ?? { ok: true, diagnostics: [], syntaxErrors: [], otherDiagnostics: [] },
        ...
        for (const fp of filePaths)
        {
            ...
            const result = compileResult !== undefined
                ...
                ? skipLint
                : await this.runLint(fp, extensionOf(fp), ctx);
        ...
        for (const fp of filePaths)
        {
            ...
            const result = await this.runFormat(fp, extensionOf(fp), ctx);
        ...
        const files: FileGateResult[] = filePaths.map((fp) => ({
            ...
            sourceContent: sourceContents[fp] ?? "",
            compile: compileResults.get(fp) ?? { ok: true, diagnostics: [], syntaxErrors: [], otherDiagnostics: [] },
    ...
    private async runCompile(filePath: string, ext: string, ctx: ActiveToolchain["ctx"]): Promise<CompileResult>
    {
        const compiler = this.toolchain.compilers.find((c) => matches(c, ext));
        ...
        try
        {
            return await compiler.compile({ filePath }, ctx);
        }
        catch (error)
        {
    ...
    private async runFormat(filePath: string, ext: string, ctx: ActiveToolchain["ctx"]): Promise<FormatResult>
    {
        const formatter = this.toolchain.formatters.find((f) => matches(f, ext));
        ...
        try
        {
            return await formatter.format({ filePath }, ctx);
        }
        catch (error)
        {
    ...
    private async runLint(filePath: string, ext: string, ctx: ActiveToolchain["ctx"], fix = true): Promise<LintResult>
    {
        const linter = this.toolchain.linters.find((l) => matches(l, ext));
        ...
        try
        {
            return await linter.lint({ filePath, fix }, ctx);
        }
        catch (error)
        {
