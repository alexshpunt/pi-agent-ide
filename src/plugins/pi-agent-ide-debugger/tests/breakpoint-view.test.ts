...
test("breakpoint view aggregates normal file reads and scopes debug source reads", async () => {
  const manager = new DebugSessionManager();
  const file = path.resolve("example.py");
  const first = manager.create({ adapter: "debugpy", program: file, args: [], cwd: process.cwd() });
