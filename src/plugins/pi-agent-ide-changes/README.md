...
`undo({ file, change: "CHANGE#..." })` restores the selected change to its `HEAD` text. If any part of that change is staged, undo also removes it from the index. `undo({ file, change: "last" })` restores the file to its state before the latest text editor invocation. `undo({ transaction: "APPLY#..." })` restores every path from one successful Apply transaction when none changed afterward. Apply receipts are single-use and live only for the current Pi session.
...
Per-file undo writes through the normal text editor pipeline and keeps path inheritance, batching, post-edit processing, the final reread, and the standard mutation diff. Apply transaction undo uses its atomic multi-path restore instead.
