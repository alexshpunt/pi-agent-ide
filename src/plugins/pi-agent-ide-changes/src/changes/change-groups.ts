...
export function createChangeGroups(
  ...
  const groups = mergedChanges.map((change) => {
    ...
    return {
      ...
      currentEndLine: lineAtOffset(
        worktreeText,
        Math.max(change.currentStart, change.currentEnd - 1),
      ),
      contextBefore,
      contextAfter,
      canonicalContent,
  ...
  return groups.map((group) => ({
    ...group,
    selector: `CHANGE#${shortestUniquePrefix(
      group.digest,
      groups.map(({ digest }) => digest),
    )}`,
  }));
}
...
export function reverseChangeGroup(worktreeText: string, group: ChangeGroup): string | undefined {
  ...
  for (const segment of [...group.segments].sort(
    (left, right) => right.currentStart - left.currentStart,
  )) {
    ...
    result =
      result.slice(0, segment.currentStart) + segment.headText + result.slice(segment.currentEnd);
  }
...
function mergeNearbyChanges(
  ...
  for (const change of changes) {
    ...
    if (
      previous !== undefined &&
      lineBreakCount(worktreeText.slice(previous.currentEnd, change.currentStart)) <=
...
function projectHeadBoundary(
  ...
  for (const change of changes) {
    ...
    const isInsertion = change.headStart === change.headEnd;

    if (isInsertion && offset === change.headStart) {
