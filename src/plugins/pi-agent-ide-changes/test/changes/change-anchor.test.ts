...
import { CHANGE_ANCHOR_KIND, ChangeTextAnchor, createChangeAnchorRegistration } from "#src/change-anchor.js";
...
describe("Git change anchors", () =>
{
    ...
    test("resolves a current selector to its first worktree line", async () =>
    {
        ...
        const result = await registration.resolver.tryResolve(groups[1]!.selector, context());
        ...
        if (result.kind === "resolved")
        {
            ...
            expect(result.anchor.value).toBe(groups[1]!.selector);
            expect(result.anchor.lineNumber).toBe(groups[1]!.currentStartLine);
        }
    });
    ...
    test("passes through wrong formats and unsupported resource sources", async () =>
    {
        ...
        await expect(registration.resolver.tryResolve("CHANGE#ABCD", context(worktreeText, "memory://example")))
    ...
    test("resolves current selectors and rejects stale ones", async () =>
    {
        ...
        await expect(registration.resolver.tryResolve(groups[0]!.selector, context())).resolves.toMatchObject({
            kind: "resolved",
        });
