...
function inspectCurrent(service: ChangeService, directory: string, file: string, worktreeText: string)
{
...
describe("Git change domain", () =>
{
    ...
    test("stages and unstages one stable change anchor", async () =>
    {
        ...
        const selector = first.groups[0]!.selector;
        expect(first.groups[0]!.state).toBe("unstaged");
        await expect(service.changeIndex(
            { source: file, worktreeText, cwd: directory },
            selector,
            "stage",
    ...
    test("replaces a partial index region with the selected worktree or HEAD state", async () =>
    {
        ...
        const selector = inspection.groups[0]!.selector;
        expect(inspection.groups[0]!.state).toBe("partial");
    ...
    test("undo removes a staged change from both states", async () =>
    {
        ...
        const selector = inspection.groups[0]!.selector;
        await service.changeIndex({ source: file, worktreeText, cwd: directory }, selector, "stage");
        const undo = await service.prepareUndo({ source: file, worktreeText, cwd: directory }, selector);
    ...
    test("undo preserves a distant change", async () =>
    {
        ...
        expect(result).toMatchObject({ status: "applied", worktreeText: headText.replace("line 18\n", "last\n") });
    });
    ...
    test("reports files where changes are not applicable or unavailable", async () =>
    {
        ...
        expect(await inspectCurrent(service, directory, path.join(directory, "new.txt"), "new\n"))
            .toEqual({ status: "not-applicable", reason: "untracked" });
        expect(await inspectCurrent(service, directory, path.join(directory, "..", "outside.txt"), "outside\n"))
            .toMatchObject({ status: "unavailable", reason: "outside-worktree" });
        expect(await inspectCurrent(service, directory, file, "binary\0text"))
