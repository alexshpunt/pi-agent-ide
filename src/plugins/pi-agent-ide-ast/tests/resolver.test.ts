...
describe("AST scope text anchors", () =>
{
    test("resolves the begin and end anchors produced for a scope", async () =>
    {
        ...
        const [scope] = await manager.getDocumentScopes(context.source, context.cwd, context.lines);
    ...
    test("resolves an occurrence suffix for repeated identical scopes", async () =>
    {
        ...
        await expect(resolver.tryResolve(repeated!.beginAnchor.value, context)).resolves.toEqual({
            kind: "resolved",
            anchor: repeated!.beginAnchor,
        });
    });
    ...
    test("rejects stale scope anchors and ignores values outside the format", async () =>
    {
        ...
        await expect(resolver.tryResolve("scope-middle-DEAD", context)).resolves.toEqual({ kind: "not-handled" });
    });
});
