    ...
    return {
        ...
        format(payload)
        {
            ...
            const lines = result.matches.flatMap((match, index) => [
                `${String(index + 1)}. ${match.file}:${String(match.range.start.line + 1)}:${
                    String(match.range.start.column + 1)
                } ${match.language}`,
                `   ${match.lines.trim()}`,
            ]);
...
function runAstGrep(pattern: string, request: SearchRequest, cwd: string, signal?: AbortSignal): Promise<AstGrepMatch[]>
{
    const args = ["run", "--pattern", pattern, "--json=compact", "--no-ignore", "parent"];

    for (const include of splitGlobs(request.include))
    {
        args.push("--globs", include);
    }

    for (const exclude of splitGlobs(request.exclude))
    {
        args.push("--globs", `!${exclude}`);
    }

    args.push(request.path ?? ".");
    return new Promise((resolve, reject) =>
    {
        const child = spawn("ast-grep", args, { cwd, stdio: ["ignore", "pipe", "pipe"] });
        let stdout = "";
        let stderr = "";
