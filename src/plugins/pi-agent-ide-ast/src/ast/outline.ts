...
const INITIALIZER_TYPES = new Set([
    "public_field_definition",
    "variable_declarator",
    "assignment",
]);
...
export class AstOutlineManager
{
    ...
    public async formatFile(filePath: string, cwd: string): Promise<SourceMappedTextContent>
    {
        ...
        try
        {
            return formatSourceViewResults([await this.readFileOutline(absolutePath, cwd)], PEEK_FORMAT_OPTIONS);
        }
        catch (error)
        {
            const message = error instanceof Error ? error.message : String(error);
            return formatSourceViewResults([failedSourceViewBlock(displayPath, message)], PEEK_FORMAT_OPTIONS);
        }
    }
    ...
    public async formatSymbol(symbol: string, cwd: string): Promise<SourceMappedTextContent>
    {
        ...
        const supportedFiles = files
            .filter((filePath) => isSupportedOutlinePath(filePath))
            .sort();
    ...
    ): Promise<SourceViewBlock>
    {
        ...
        const renderedLines = projectedLines.map((line) =>
        {
            ...
            const sourceContent = sourceLineNumber === undefined ? undefined : sourceLines[sourceLineNumber - 1] ?? "";
            const markers = sourceLineNumber === undefined ? undefined : markersByLine.get(sourceLineNumber);
...
function unwrapDeclaration(node: WTS.Node): WTS.Node
{
    ...
    if (
        parent
        && (parent.type === "export_statement" || parent.type === "decorated_definition"
...
function renderNodeLines(node: WTS.Node, source: string, extension: string): ProjectedLine[]
{
    ...
    while (mapped.length > 0 && /\s/u.test(mapped.at(-1)!.char))
    {
        mapped.pop();
    }
...
function mapReplacement(replacement: Replacement): MappedCharacter[]
{
    ...
    {
        const char = replacement.text[index]!;
...
function collectDataReplacements(node: WTS.Node, extension: string, replacements: Replacement[]): void
{
    for (const child of node.namedChildren)
    {
...
{
    const scalarTypes = extension === ".json" || extension === ".jsonc"
        ? ["string", "number", "true", "false", "null"]
        : extension === ".yaml" || extension === ".yml"
        ? ["string_scalar", "integer_scalar", "float_scalar", "boolean_scalar", "null_scalar", "block_scalar"]
        : [
            "string",
            "integer",
            "float",
...
function isDataKey(node: WTS.Node): boolean
{
    ...
    while (ancestor && !ancestor.type.endsWith("pair") && ancestor.type !== "pair" && ancestor.type !== "table")
    {
...
{
    return DECLARATION_TYPES.has(node.type)
...
function findBody(node: WTS.Node): WTS.Node | undefined
{
    ...
    return node.namedChildren.find((child) =>
        child.type === "block" || child.type === "compound_statement" || child.type === "statement_block"
    );
}
...
function renderBodyPlaceholder(body: WTS.Node, extension: string): BodyPlaceholder
{
    ...
    const interiorHasContent = bodyLines.slice(1, -1).some((line) => line.trim() !== "");
    const firstHasContent = afterOpen.trim() !== "";
    const lastHasContent = beforeClose.trim() !== "";

    if (!interiorHasContent && !firstHasContent && !lastHasContent)
    {
        return {
            text: body.text,
    ...
    output.push(firstHasContent ? firstLine.slice(0, openIndex + 1) + maskSegment(afterOpen) : firstLine);
    lineSourceLines.push(startLine);

    if (!firstHasContent && interiorHasContent)
    {
        const indentation = bodyLines.slice(1, -1).find((line) => line.trim() !== "")?.match(/^\s*/u)?.[0] ?? "";
        output.push(`${indentation}…`);
        lineSourceLines.push(undefined);
    }

    output.push(lastHasContent ? maskSegment(beforeClose) + lastLine.slice(closeIndex) : lastLine);
