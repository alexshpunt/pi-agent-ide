...
import type { ReadPostReadHandler, ReadTextLine, ReadToolResult } from "pi-agent-read/api/tools/read";
...
export function createAstScopePostReadHandler(): ReadPostReadHandler
{
    return (context) =>
    {
        ...
        const appendedLines = [...contextLines.values()].sort((left, right) => left.lineNumber - right.lineNumber);
        ...
        return {
            ...
            ...(markers === undefined
                ...
                : {
                    presentation: {
                        ...line.presentation,
                        suffix: `${line.presentation?.suffix ?? ""}${
...
): ReadToolResult
{
    ...
    const separator = lastRenderedLine?.lineEnding !== undefined && lastRenderedLine.lineEnding.length > 0
        ...
        : "\n";
    const appendedText = appendedLines.map((line, index) =>
    {
