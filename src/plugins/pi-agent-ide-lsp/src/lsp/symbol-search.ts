...
function toHit(cwd: string, symbol: LspWorkspaceSymbol, uri: string, line: number, character: number): SymbolHit
{
...
): Promise<SymbolHit[]>
{
    ...
    for (const symbol of definitions)
    {
        ...
        const references = await requestReferences(opened.client, opened.uri, definition.range.start).catch(() => []);
