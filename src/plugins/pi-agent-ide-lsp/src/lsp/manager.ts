...
export class LspManager
{
    ...
    ): Promise<LspClient | null>
    {
        ...
        const resolved = this._registry.resolve(ext);
        const match = capability === "symbols"
            ? resolved[0]
    ...
    {
        const startingClient = client ?? new LspClient({
            serverId: match.serverId,
            rootUri,
            command: match.config.command,
            ...(match.config.initOptions ? { initOptions: match.config.initOptions } : {}),
        });
    ...
    async getWorkspaceClients(cwd: string, capability: "symbols" = "symbols"): Promise<LspClient[]>
    {
        ...
        for (const ext of this._registry.knownExtensions)
        {
            try
            {
                const client = await this.getOrStart(ext, cwd, capability);
    ...
    private _subscribeToPushDiagnostics(clientKey: string, client: LspClient): void
    {
        ...
        {
            const notification = parsePushNotification(params);

            if (
                !notification || client.diagnosticMode !== "push" || client.hasActiveDiagnosticRequest(notification.uri)
            )
            {
                return;
            }
            ...
            if (
                notification.version !== undefined && currentVersion !== undefined
...
function parsePushNotification(params: unknown): {
    uri: string;
    version?: number;
...
} | undefined
{
    ...
    const value = params as { uri?: unknown; version?: unknown; diagnostics?: unknown; };
...
{
    return {
        code: typeof diagnostic.code === "number" ? String(diagnostic.code) : (diagnostic.code ?? "LSP"),
