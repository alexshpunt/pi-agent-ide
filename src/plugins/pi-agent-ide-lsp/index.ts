...
export default async function registerLsp(pi: ExtensionAPI): Promise<void>
{
    ...
    const readPlugin = {
        ...
        setup(api)
        {
            api.addResolver({ resolver: createLspDiagnosticResolver(managerFor), renderResult: renderReadResult });
            api.addResolver({ resolver: createLspSymbolResolver(managerFor), renderResult: renderReadResult });
            api.addResolver({ resolver: createLspGraphResolver(managerFor), renderResult: renderReadResult });
            api.addHandler({
                stage: "read",
...
): ResourceResolutionAttempt
{
    ...
    return {
        ...
        resource: {
            ...
            async read()
            {
                const [text, manager] = await Promise.all([
    ...
    return {
        ...
        async present(document, context)
        {
            ...
            try
            {
                ...
                const compiled = await createLspCompiler(manager).compile({ filePath: source }, { cwd: context.cwd });
...
async function loadRegistry(cwd: string): Promise<LspServerRegistry>
{
    ...
    try
    {
        return await LspServerRegistry.fromPackageDir(configDirectory);
    }
    catch
    {
        return LspServerRegistry.fromConfig({ servers: {} });
    }
}
