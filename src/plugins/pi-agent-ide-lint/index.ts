...
export default async function registerLint(pi: ExtensionAPI): Promise<void> {
  ...
  await Promise.all([
    ...
    connectDoctorPlugin(pi, lintDoctorPlugin),
  ]);
}
...
function resolveLintDiagnosticSource(
  ...
  return {
    ...
    resource: {
      ...
      async read() {
        const [text, registry] = await Promise.all([readFile(filePath, "utf8"), getRegistry(cwd)]);
        const linted = await createCommandLinter(registry).lint({ filePath, fix: false }, { cwd });
