...
assert(sourceManifest.name === "pi-agent-ide", `Expected pi-agent-ide, received ${sourceManifest.name}`);
assert(typeof releaseVersion === "string" && /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(releaseVersion), "Invalid package version");
...
copyRuntimeTree(join(repositoryRoot, "docs"), join(stageDirectory, "docs"), { includeDocumentation: true });
...
assert(packResult?.filename, "npm pack did not return a tarball name");
...
function createReleaseManifest()
{
    ...
    for (const owner of [sourceManifest, ...internalPackages.map((entry) => entry.manifest)])
    {
        ...
        {
            if (piPeerNames.has(name) || name === sourceManifest.name || internalNames.has(name)) continue;
    ...
    manifest.dependencies = Object.fromEntries([...dependencies].sort(([left], [right]) => left.localeCompare(right)));
    manifest.peerDependencies = Object.fromEntries([...piPeerNames].sort().map((name) => [name, "*"]));
...
function sanitizeInternalManifest(source)
{
    ...
    for (const [name, specifier] of Object.entries(manifest.dependencies ?? {}))
    {
        ...
        {
            assert(!String(specifier).startsWith("workspace:"), `${manifest.name} has an external workspace dependency on ${name}`);
    ...
    if (Object.keys(peerDependencies).length > 0) manifest.peerDependencies = sortRecord(peerDependencies);
...
function copyRuntimeTree(source, target, options = {})
{
    ...
    {
        if (currentSource !== sourceRoot && options.stopAtPackageBoundaries && packageDirectories.has(resolve(currentSource))) return;
...
function shouldSkipFile(name, _relativePath, options)
{
    if (name.endsWith(".md") && options.includeDocumentation !== true) return true;
    return name === "lefthook.yml"
        || name === "pnpm-lock.yaml"
...
function validatePackage(packageRoot, packResult, tarballPath)
{
    const paths = [];
    walk(packageRoot, (path) => paths.push(toPosix(relative(packageRoot, path))));
    const topLevel = [...new Set(paths.map((path) => path.split("/")[0]))].sort();
    ...
    assert(packagedManifest.devDependencies === undefined, "Release manifest contains devDependencies");
    assert(packagedManifest.pi?.extensions?.length === 1, "Release manifest must expose one Pi extension");
    assert(packagedManifest.pi.extensions[0] === "./src/pi-agent-ide.ts", "Unexpected Pi extension entrypoint");
    ...
    for (const markdownPath of paths.filter((path) => path.endsWith(".md")))
    {
        ...
        for (const match of markdown.matchAll(/\[[^\]]+\]\(([^)]+)\)/g))
        {
            ...
            const resolvedTarget = resolve(dirname(join(packageRoot, markdownPath)), decodeURIComponent(target));
            assert(resolvedTarget.startsWith(`${packageRoot}${sep}`) && existsSync(resolvedTarget), `${markdownPath} links to missing ${target}`);
        }
    }
    ...
    {
        assert(packagedManifest.dependencies?.[name] === releaseVersion, `Missing bundled dependency ${name}`);
        assert(packagedManifest.bundledDependencies?.includes(name), `Missing bundle declaration for ${name}`);
        assert(existsSync(join(packageRoot, "node_modules", name, "package.json")), `Missing bundled package ${name}`);
    }
    ...
    for (const manifestPath of manifestPaths)
    {
        ...
        assert(!/(?:workspace:|file:|\/root\/)/.test(manifestText), `${manifestPath} contains a development path or range`);
        for (const name of piPeerNames)
        {
            if (manifest.peerDependencies?.[name] !== undefined)
            {
                assert(manifest.peerDependencies[name] === "*", `${manifestPath} must use a star Pi peer for ${name}`);
            }
        }
    }

    assert(packResult.unpackedSize < 25_000_000, `Tarball is unexpectedly large: ${packResult.unpackedSize} bytes`);
    return {
...
{
    return path.split("/").some((segment) => segment !== "node_modules" && forbiddenDirectoryNames.has(segment));
}
