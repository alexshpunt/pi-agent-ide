...
describe("text anchor runtime contracts", () =>
{
    test("accepts structural resolvers and all fulfilled attempt kinds", () =>
    {
        ...
        expect(isTextAnchorResolutionAttempt({
            kind: "resolved",
            anchor: new FixtureTextAnchor("fixture", 2),
        })).toBe(true);
        expect(isTextAnchorResolutionAttempt({ kind: "failed", error: new Error("failed") })).toBe(true);
    });
    ...
    test("rejects malformed resolver and attempt values", () =>
    {
        ...
        expect(isTextAnchorResolutionAttempt({ kind: "resolved", anchor: { value: "fixture", lineNumber: 2 } }))
            .toBe(false);
