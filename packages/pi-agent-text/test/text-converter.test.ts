...
test("preserves strict UTF-8 text, multiline endings, and empty content", async () => {
  ...
  await expect(
    converter.tryConvert({ source: "notes", bytes: encoder.encode(value) }, {}),
  ).resolves.toEqual({
    kind: "converted",
    content: [{ type: "text", text: value }],
  });
  await expect(
    converter.tryConvert({ source: "empty", bytes: new Uint8Array(0) }, {}),
  ).resolves.toEqual({
    kind: "converted",
...
test("fails malformed declared text and declines unknown malformed binary", async () => {
  ...
  await expect(converter.tryConvert({ source: "unknown", bytes }, {})).resolves.toEqual({
    kind: "not-handled",
  });
});
