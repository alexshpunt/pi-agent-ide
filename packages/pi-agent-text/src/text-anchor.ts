...
export abstract class TextAnchor
{
    ...
    public static is(value: unknown): value is TextAnchor
    {
        ...
        try
        {
            ...
            return candidate[textAnchorBrand] === true
...
export interface TextAnchorResolver
{
    ...
    tryResolve(value: string, context: TextAnchorResolverContext): Promise<TextAnchorResolutionAttempt>;
}
...
{
    return safeCheck(resolverSchema, value)
        && safeEvaluate(() => (value as TextAnchorResolver).id.trim().length > 0);
}

export function isTextAnchorResolutionAttempt(value: unknown): value is TextAnchorResolutionAttempt
{
    return safeCheck(attemptSchema, value)
        && safeEvaluate(() =>
        {
