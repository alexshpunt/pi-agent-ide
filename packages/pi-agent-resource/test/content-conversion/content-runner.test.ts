...
test("runs converters serially by stable priority and stops after conversion", async () =>
{
    ...
    runner.register(registration("late", 20, async () =>
    {
        calls.push("late");
        return { kind: "converted", content: [{ type: "text", text: "done" }] };
    }));
    runner.register(registration("first-equal", 10, async () =>
    {
    ...
    runner.register(registration("second-equal", 10, async () =>
    {
        calls.push("second-equal");
        return { kind: "not-handled" };
    }));
    runner.register(registration("never", 30, async () =>
    {
        calls.push("never");
...
test("treats failed and malformed outcomes as terminal", async () =>
{
    ...
    failedRunner.register(registration("failed", 0, async () => ({ kind: "failed", error: failure })));
    failedRunner.register(registration("fallback", 1, async () =>
    {
        fallbackCalls += 1;
    ...
    malformedRunner.register(registration("malformed", 0, async () => (
        { kind: "converted", content: [] } as unknown as ContentConversionAttempt
    )));
    malformedRunner.register(registration("fallback", 1, async () =>
    {
        fallbackCalls += 1;
        return { kind: "converted", content: [{ type: "text", text: "fallback" }] };
    }));
...
test("stops for cancellation and reports unsupported content", async () =>
{
    ...
    runner.register(registration("aborting", 0, async () =>
    {
        controller.abort();
        return { kind: "not-handled" };
    }));
    runner.register(registration("never", 1, async () =>
    {
        secondCalls += 1;
        return { kind: "converted", content: [{ type: "text", text: "wrong" }] };
    }));
...
function converter(
    id: string,
    tryConvert: ContentConverter["tryConvert"],
): ContentConverter
{
