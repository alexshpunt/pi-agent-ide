    ...
    return {
        ...
        async tryConvert(input, context)
        {
            ...
            catch (error)
            {
                return {
                    kind: "failed",
                    error: cancellationError(context.signal)
                        ?? new TypeError(
                            `Unable to read PDF document ${input.source}: ${errorMessage(error)}`,
                            { cause: error },
                        ),
                };
            }
...
export async function createPdfContent(bytes: Uint8Array, signal?: AbortSignal): Promise<AgentContent>
{
