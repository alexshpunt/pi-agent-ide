...
test("does not claim unrelated bytes and rejects malformed declared PDFs", async () =>
{
    ...
    await expect(converter.tryConvert({ source: "plain.txt", bytes }, {}))
        .resolves.toEqual({ kind: "not-handled" });
    expect((await converter.tryConvert({ source: "broken.pdf", mediaType: "application/pdf", bytes }, {})).kind)
        .toBe("failed");
    expect((await converter.tryConvert({ source: "broken.pdf", bytes: new TextEncoder().encode("%PDF-broken") }, {})).kind)
        .toBe("failed");
});
...
test("honors cancellation", async () =>
{
    ...
    controller.abort();
    const outcome = await createPdfContentConverter().tryConvert({ source: "document.pdf", bytes: pdf }, {
