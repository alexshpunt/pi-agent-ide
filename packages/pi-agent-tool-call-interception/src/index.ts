...
import type { AgentToolResult, Theme, ThemeColor, ToolDefinition } from "@earendil-works/pi-coding-agent";
...
export type ToolCallAnnotation =
    | { readonly kind: "stale-anchor"; readonly label?: string; readonly reason?: string; readonly color?: ThemeColor; }
    | { readonly kind: "blocked"; readonly label?: string; readonly reason: string; readonly color?: ThemeColor; }
    | { readonly kind: "aborted"; readonly label?: string; readonly reason?: string; readonly color?: ThemeColor; }
    | { readonly kind: "custom"; readonly label: string; readonly reason?: string; readonly color: ThemeColor; };
...
export function withToolCallInterceptionRendering<
...
): ToolDefinition<TParams, TDetails, TState>
{
    ...
    return {
        ...
        renderCall(args, theme, context)
        {
            ...
            const renderArgumentPatch = annotations.getRenderArgumentPatch(context.toolCallId)
                ?? state[RENDER_ARGUMENT_PATCH];
            ...
            const effectiveArgs = renderArgumentPatch === undefined
                ? args
                : { ...renderArgumentPatch, ...args } as typeof args;
            ...
            const previous = context.lastComponent instanceof AnnotatedToolCall
            ...
            try
            {
                component = renderCall?.(effectiveArgs, theme, {
                    ...context,
                    args: effectiveArgs,
        ...
        renderResult(result, options, theme, context)
        {
            ...
            const effectiveContext = renderArgumentPatch === undefined
                ? context
            ...
            const annotation = persisted ?? (options.isPartial || context.isError ? transient : undefined);
...
{
    const stale = annotation.kind === "stale-anchor";
    const icon = stale ? "⚠" : "⊘";
    const label = annotation.label ?? (stale ? "Stale Anchor" : "Blocked");
    const color = annotation.color ?? (stale ? "warning" : "error");
